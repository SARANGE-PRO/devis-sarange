...
const buildTableBody = (cartItems, isMultiTva, tvaRate) => {
  ...
  cartItems.forEach((item) => {
    ...
    const productRow = [
      ...
      {
        ...buildPriceCellData(calc.unitPriceAfterDiscount, pricing.originalUnitHT, 8.4, true),
        styles: { halign: 'right' },
      }
    ];
...
export const buildQuotePdfDocument = async (
  ...
  // Pre-fetch assets
  const [logoDataUrl, wasteIconDataUrl, footerLogoDataUrl, productImageAssetsList] =
    await Promise.all([
      getLogoDataUrl(),
      getWasteIconDataUrl(),
