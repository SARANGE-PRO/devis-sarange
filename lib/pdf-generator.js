...
export const generateQuotePDF = async (clientData, cartItems, tvaRate, quoteSettings = null) => {
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data) => {
      ...
      if (data.section === 'body' && data.column.index === 0) {
        ...
        if (item.customImage) {
          const renderTask = getImageAsset(item.customImage)
            .then((imageAsset) => {
              if (!imageAsset?.dataUrl) return;
              ...
              data.doc.addImage(imageAsset.dataUrl, 'PNG', imgX, imgY, imgW, imgH);
            })
            .catch(() => null);
          deferredIconRenders.push(renderTask);
          return;
