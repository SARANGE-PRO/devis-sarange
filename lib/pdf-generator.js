...
const PAYMENT_TERMS = [
  ...
  "Acompte de 50 % à la commande, ou par chèque ou virement lors de la prise de côtes si un métré SARANGE est prévu.",
  "Solde à la réception des travaux ou à l'enlèvement.",
...
const getDynamicPaymentTerms = (quoteSettings) =>
  quoteSettings ? buildPaymentTermsForPdf(quoteSettings) : PAYMENT_TERMS;

const getDynamicCgvSections = (quoteSettings) =>
  CGV_SECTIONS.map((section) =>
    section.title === 'Article 2 - Prix et conditions de paiement'
      ? {
        ...section,
        text:
          `2.1. Prix : nos prix sont fermes pendant la durée de validité indiquée sur le devis, soit ${getValidityLabel(quoteSettings)} à compter de la date d'émission. ` +
          `${buildPaymentLegalParagraph(quoteSettings)} ` +
          `2.3. Retard de paiement : tout retard de paiement entraîne de plein droit, sans mise en demeure préalable, l'application de pénalités calculées au taux de refinancement de la BCE majoré de 10 points, l'exigibilité d'une indemnité forfaitaire de 40 € pour frais de recouvrement pour les clients professionnels, ainsi que la suspension immédiate de nos obligations jusqu'au règlement intégral.`,
      }
      : section
  );
...
const drawCommercialAndTotals = (doc, context, startY) => {
  ...
  const summaryRows = [];
  const paymentTerms = getDynamicPaymentTerms(quoteSettings);
  const paymentMilestones = getPaymentMilestones(quoteSettings, totals.totalTTC);
...
const drawCgvPages = (doc, context, cgvPages) => {
  ...
  const sections = getDynamicCgvSections(context.quoteSettings);
...
export const buildMultiVariantQuotePdfDocument = async (clientData, variants, options = {}) => {
  ...
  for (let index = 0; index < safeVariants.length; index += 1) {
    ...
    const built = await buildQuotePdfDocument(
      ...
      { ...sharedOptions, variantLabel: label }
    );
