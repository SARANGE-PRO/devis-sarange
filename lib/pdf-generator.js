...
const PANEL_SLOT_LABEL = 'Panneau decoratif : ';
...
const buildTableBody = (cartItems, isMultiTva, tvaRate) => {
  ...
  cartItems.forEach((item) => {
    ...
    // Porte à panneau décoratif : on ajoute une dernière ligne « repère » (libellé seul)
    // dont la position sera mémorisée pour y tamponner le panneau choisi à la signature.
    const panelLineId = String(item.id || item.leafId || '').trim();
    const isDecoDoor =
      Boolean(panelLineId) &&
      (item.panneauDecoratif === true || item.options?.panneauDecoratif === true);
    if (isDecoDoor) {
      fullText = `${fullText}\n${PANEL_SLOT_LABEL}${PANEL_SLOT_PLACEHOLDER}`;
    }
    ...
    const productRow = [
      ...
      },
      {
        ...
        isDesignation: true,
        isDeco: isDecoDoor,
        lineId: isDecoDoor ? panelLineId : null,
      },
      {
    ...
    didDrawCell: (data) => {
      ...
      if (data.section === 'body' && data.column.index === 1 && data.cell.raw && data.cell.raw.isDesignation) {
        ...
        logicalLines.forEach((lLine, index) => {
          ...
          // Repère « panneau décoratif » : dernière ligne d'une porte déco. On mémorise
          // la position juste après le libellé pour y tamponner le panneau à la signature.
          if (
            cell.raw.isDeco &&
            cell.raw.lineId &&
            index === logicalLines.length - 1 &&
            lLine.startsWith('Panneau')
          ) {
            const labelWidthMm = doc.getTextWidth(PANEL_SLOT_LABEL);
