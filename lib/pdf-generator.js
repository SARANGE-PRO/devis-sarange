...
const renderMenuiserieToDataURL = (item) => {
  ...
  try {
    if (item.isComposite && (item.composition?.length || item.modules?.length)) {
      ...
      parentCanvas.height = totalHeight;
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data) => {
      ...
      if (data.section === 'body' && data.column.index === 0) {
        ...
        if (imgData) {
          const config = item.isComposite
            ? {
              width: item.widthMm || getCompositeDimensions(item.composition, item.modules).width,
              height: item.heightMm || getCompositeDimensions(item.composition, item.modules).height,
            }
            : buildMenuiserieConfig(item);
