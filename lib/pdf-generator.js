...
import { WASTE_ICON_SVG } from './assets';
...
const slugify = (value, fallback = 'client') => {
  const normalized = sanitizePdfText(value || fallback)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '');
...
const buildQuoteNumber = (date) => {
  ...
  return `DV-${year}${month}${day}-${hours}${minutes}${seconds}`;
};
...
const getItemPricingSummary = (item, calc) => {
  ...
  cartItems.forEach((item) => {
    ...
    if (item.productId === 'gestion-dechets') {
      ...
      fullText = sanitizePdfText(item.productLabel || item.customLabel || 'Produit sur mesure') + '\n' +
                 sanitizePdfText(item.customDescription || '');
    } else {
      const designation = generateDesignation(item, calc, pricing);
...
export const generateQuotePDF = async (clientData, cartItems, tvaRate) => {
  ...
  const baseFilename = `Devis_SARANGE_${quoteNo}`;
  const clientRef = clientData?.referenceDevis ? `_${slugify(clientData.referenceDevis, '')}` : '';
  const clientName = `_${slugify(clientData?.nom, 'sans_nom')}`;
  const filename = `${baseFilename}${clientRef}${clientName}.pdf`;
  doc.save(filename);
};
