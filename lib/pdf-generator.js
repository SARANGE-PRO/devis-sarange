...
const renderMenuiserieToDataURL = (item) => {
  ...
  try {
    if (item.isComposite && (item.composition?.length || item.modules?.length)) {
      const composition = normalizeCompositeComposition(item.composition, item.modules);
      const dimensions = getCompositeDimensions(composition);
      ...
      let currentY = coffreH;

      composition.forEach((row) => {
        const rowHeight = row.modules.reduce(
          (maxHeight, module) =>
            Math.max(maxHeight, parseInt(module.heightMm ?? module.hauteur, 10) || 0),
          0
        );
        let currentX = 0;
        ...
        row.modules.forEach((module) => {
          const moduleItem = {
            ...
            sheetName: module.sheetName || getProductById(module.productId)?.sheet,
            widthMm: parseInt(module.widthMm ?? module.largeur, 10) || 0,
            heightMm: parseInt(module.heightMm ?? module.hauteur, 10) || rowHeight,
            glazingId:
              module.glazingId ??
              module.options?.glazingId ??
              module.glazingOption?.id ??
              item.glazingOption?.id,
            hasSousBassement:
              module.hasSousBassement ?? module.options?.hasSousBassement ?? false,
            sousBassementHeight:
              module.sousBassementHeight ?? module.options?.sousBassementHeight ?? 0,
            petitsBoisH:
              module.petitsBoisH ?? module.options?.petitsBoisH ?? 0,
            petitsBoisV:
              module.petitsBoisV ??
              module.options?.petitsBoisV ??
              (module.options?.petitsBoisH == null ? module.options?.petitsBois : 0) ??
              0,
            sashOptions: module.sashOptions ?? module.options?.sashOptions ?? {},
            openingDirection:
              module.openingDirection ?? module.options?.openingDirection ?? 'standard',
            panneauDecoratif:
              module.panneauDecoratif ?? module.options?.panneauDecoratif ?? false,
            handleHeightMm:
              module.handleHeightMm ?? module.options?.handleHeightMm ?? null,
            svgColor: module.svgColor,
          };
          ...
          parentContext.drawImage(
            moduleCanvas,
            currentX,
            currentY + Math.max(0, rowHeight - config.height)
          );
      ...
      if (monobloc) {
        drawCompositeMonoblocOverlay(parentContext, {
          ...
          coffreH,
          scale: monoScale,
          frameColor: composition[0]?.modules?.[0]?.svgColor || '#FFFFFF',
...
export const buildQuotePdfDocument = async (
        ...
        if (imgData) {
          ...
          if (item.isComposite) {
            const compositeWidth =
              item.widthMm || getCompositeDimensions(item.composition, item.modules).width;
            const compositeHeight =
              item.heightMm || getCompositeDimensions(item.composition, item.modules).height;
  ...
  return {
    ...
    signatureAnchors: context.signaturePdfAnchors || null,
  };
};
