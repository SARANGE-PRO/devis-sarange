...
const buildPorteDesignation = (item, pricing) => {
  ...
  if (item.panneauDecoratif) {
    lines.push(`Porte d'entrée ${materialLabel}${isReno ? ' – Rénovation' : ' – Neuf'}`);
  } else {
    lines.push(`Porte d'entrée vitrée ${materialLabel}${isReno ? ' – Rénovation' : ' – Neuf'}`);
  }
  ...
  lines.push(
    isAlu
      ? 'Profilé aluminium 60 mm Schüco – rupture de pont thermique – seuil PMR'
      : 'Profilé 70 mm PVC Schüco – 5 chambres – seuil PMR'
  );

  // ── 5. Sécurité ─────────────────────────────────────────────────────────────
  lines.push('Sécurité : serrure 5 points – paumelles renforcées – renforts acier');

  // ── 6. Poignée / barre de tirage ────────────────────────────────────────────
  lines.push(`Barre de tirage – ${getHandleHeightDescription(item.handleHeightMm, item.heightMm, item.allegeHeightMm)}`);
