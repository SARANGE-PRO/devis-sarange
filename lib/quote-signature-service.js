...
const readStagedQuoteDeliveryPdf = async (userId, uploadId) => {
  if (!STAGED_UPLOAD_ID_PATTERN.test(String(uploadId || ''))) {
    throw createHttpError('Le PDF source est manquant.', 400);
  }
...
export const previewQuoteDelivery = async ({ userId, quoteId, deliveryMode, pdfInfo, variants }) => {
  ensureServerConfig({
    requireEmail: true,
    requirePublicUrl: deliveryMode === 'signature',
  });
  ...
  return {
    ...
    text,
    attachments: [{ filename: originalFilename }],
...
export const previewQuoteSignatureReminder = async ({ sessionId, reminderLevel }) => {
  ...
  return {
    ...
    attachments: session.document?.originalFilename
      ? [{ filename: session.document.originalFilename }]
      : [],
