...
// Portes à panneau décoratif d'un devis : le client choisit son modèle de panneau
// au moment de la signature (sélecteur en iframe, same-origin). On dérive UNE entrée
// par ligne concernée à partir des articles du devis (présents au moment de l'envoi).
const buildPanelSelections = (quoteData) => {
  const items = Array.isArray(quoteData?.payload?.cartItems)
    ? quoteData.payload.cartItems
    : [];
  const selections = [];
  items.forEach((item, index) => {
    ...
    if (!isDeco) return;
    selections.push({
      lineId: normalizeEnv(item?.id) || normalizeEnv(item?.leafId) || `porte-${index + 1}`,
      productLabel:
        normalizeEnv(item?.productLabel) ||
        normalizeEnv(item?.sheetName) ||
        "Porte d'entrée",
      repere: normalizeEnv(item?.repere),
      widthMm: Number(item?.widthMm) || null,
      heightMm: Number(item?.heightMm) || null,
      colorId: normalizeEnv(item?.colorOption?.id) || normalizeEnv(item?.colorOptionId),
      colorLabel: normalizeEnv(item?.colorOption?.label) || normalizeEnv(item?.colorOptionLabel),
    });
  });
  return selections;
};
...
const buildSessionPayload = ({
  ...
  return {
    ...
    panelSelections: buildPanelSelections(quoteData),
    document: {
      originalPdfPath,
      originalFilename,
...
export const signQuoteSignatureSession = async ({
  ...
  const panelSelections = Array.isArray(context.session.panelSelections)
