...
const getTransferBlockHtml = ({
  ...
  const bankBoxHtml = `
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background: #ffffff; border: 1px dashed #cbd5e1; border-radius: 12px; margin-bottom: 15px;">
      <tr>
        <td style="padding: 16px 16px 12px 16px; text-align: center;">
          ...
          <p style="margin: 0; font-family: 'Courier New', monospace; font-size: 15px; font-weight: 700; color: #0f172a; letter-spacing: 0.4px; line-height: 1.65; word-break: break-word;">${escapeHtml(BANK_IBAN)}</p>
        </td>
      </tr>
      <tr>
        <td style="padding: 0 16px 12px 16px;">
          <div style="background-color: #f8fafc; border-radius: 10px; padding: 12px 14px;">
  ...
  return `
    <div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; margin-bottom: 10px;">
...
const buildQuoteModel = (quoteData, pdfInfo) => {
  ...
  return {
    ...
    reference: normalizeEnv(
      quoteData?.referenceDevis || quoteData?.payload?.clientData?.referenceDevis
    ),
    issueDate,
    ...
    hasMeasurementVisit,
    payment: buildPaymentModel(quoteData, totalTTC),
  };
};
    ...
    contentHtml: `
        ...
        <td style="padding: 40px 30px;">
          ...
          <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-left: 4px solid #22c55e; padding: 25px; margin: 35px 0; border-radius: 8px;">
            <h3 style="margin-top: 0; margin-bottom: 20px; color: #0f172a; font-size: 17px;">
              🚀 La suite de votre projet${hasMeasurementVisit ? ' en 2 étapes' : ''} :
            </h3>

            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom: 20px;">
              <tr>
                <td width="40" valign="top">
                  <div style="background-color: #f97316; color: white; width: 28px; height: 28px; border-radius: 50%; text-align: center; line-height: 28px; font-size: 14px; font-weight: bold;">1</div>
                </td>
                <td valign="top">
                  <p style="margin: 0 0 8px 0; color: #334155; font-size: 16px;"><strong>Règlement de l'acompte</strong></p>
                  <p style="margin: 0 0 10px 0; color: #475569; line-height: 1.5; font-size: 15px;">${depositSettlementNotice}</p>
                  ${getTransferBlockHtml({
      session,
      ...
      showAmounts: true,
        ...
        ? `<table width="100%" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <td width="40" valign="top">
                        <div style="background-color: #f97316; color: white; width: 28px; height: 28px; border-radius: 50%; text-align: center; line-height: 28px; font-size: 14px; font-weight: bold;">2</div>
                      </td>
                      <td valign="top">
                        <p style="margin: 0 0 8px 0; color: #334155; font-size: 16px;"><strong>Prise de mesures (Métré)</strong></p>
                        <p style="margin: 0; color: #475569; line-height: 1.5; font-size: 15px;">Notre expert technique vous contactera très rapidement pour fixer le rendez-vous de métré définitif à votre domicile. C'est la garantie d'une installation parfaite ! 📏</p>
                      </td>
                    </tr>
                  </table>`
        : ''
...
const toPublicSessionResponse = (session, { active = true } = {}) => ({
  ...
  requiresReducedVatAck: session.document?.signatureAnchors?.reducedVatRequired === true,
...
export const signQuoteSignatureSession = async ({
  ...
  if (
    context.session.document?.signatureAnchors?.reducedVatRequired === true &&
  ...
  const originalPdfBuffer = await downloadPdfBuffer(originalPdfPath);
  const signedAt = new Date();
  const signedPdfBuffer = await applySignatureToPdf({
    originalPdfBuffer,
    signatureDataUrl,
    signatureAnchors: context.session.document?.signatureAnchors,
    signedAt,
    signerName,
    acceptReducedVat,
  });

  const signedFilename = buildSignedPdfFilename(context.session.document?.originalFilename);
