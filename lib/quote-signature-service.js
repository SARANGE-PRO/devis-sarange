...
import { emitQuoteSignedEvent } from './financial-events.mjs';
import { buildPanelSelections } from './panel-selections.mjs';
import { htmlToPlainText } from './email-plain-text.mjs';
import {
  CUSTOM_MESSAGE_PLACEHOLDER,
  customMessageToHtml,
  resolveCustomMessage,
  resolveCustomSubject,
} from './email-custom-message.mjs';
import {
  ATTACHMENT_LIMITS,
...
} from './email-recipients.mjs';
import {
  buildCheckmarkSegments,
  drawAtAnchor,
  toPdfPoints,
  topMmToPdfY,
} from './pdf-signature-anchors.mjs';
...
import { getTaxRegimeValidation } from '@/lib/tax-regime.mjs';
import {
  DEFAULT_SIGNATURE_EXPIRY_DAYS,
  buildSignatureAttachmentHref,
...
// Zone de transit des PDF téléversés en morceaux AVANT l'envoi : l'hébergement
// limite chaque requête HTTP à ~4,5 Mo, un devis multi-variantes (comparatif +
// un PDF signable par variante) ne peut donc pas voyager en une seule requête.
const STAGING_ROOT = 'quote-signature-uploads';
const STAGED_UPLOAD_ID_PATTERN = /^up_[a-f0-9]{32}$/;
const STAGED_UPLOAD_MAX_CHUNKS = 24;
const STAGED_CHUNK_MAX_BYTES = 3 * 1024 * 1024;
const STAGED_PDF_MAX_BYTES = 25 * 1024 * 1024;
...
const getDepositSettlementNotice = (hasMeasurementVisit) =>
  hasMeasurementVisit
    ? "L'acompte peut être réglé par virement à la commande ou, lors de la prise de côtes, par chèque ou virement."
    : "L'acompte peut être réglé à la commande par virement ou par chèque.";

// Échéancier transmis par le client au moment de l'envoi : celui du PDF
// (ventilation fabrication/pose comprise). Sanitizé avant persistance.
const sanitizePaymentMilestones = (raw) =>
  (Array.isArray(raw) ? raw : [])
    .map((milestone) => ({
      id: normalizeEnv(milestone?.id),
      label: normalizeEnv(milestone?.label),
      percent: Number(milestone?.percent) || 0,
      amountTTC: Number(milestone?.amountTTC) || 0,
      dueLabel: normalizeEnv(milestone?.dueLabel),
    }))
    .filter((milestone) => milestone.label && Number.isFinite(milestone.amountTTC));

const buildPaymentModel = (quoteData, totalTTC, providedMilestones = null) => {
  const settings = normalizeQuoteSettings(quoteData?.payload?.quoteSettings || {});
  // Priorité aux échéances calculées avec le PDF (exactes, ventilation
  // comprise) ; à défaut, recalcul depuis les réglages (fiable pour les modes
  // standard/personnalisé — l'acompte reste correct dans tous les cas).
  const sanitizedProvided = sanitizePaymentMilestones(providedMilestones);
  const milestones = sanitizedProvided.length
    ? sanitizedProvided
    : getPaymentMilestones(settings, totalTTC);
...
const getTransferBlockHtml = ({
  ...
  // Échéancier COMPLET : le contrat ne se limite pas au premier acompte.
  // Lorsque les échéances du devis sont connues (mode personnalisé ou
  // fabrication/pose), elles sont toutes affichées ; sinon, l'acompte est
  // accompagné d'un renvoi explicite vers l'échéancier du devis.
  const milestones = Array.isArray(payment.milestones) ? payment.milestones : [];
  const showFullSchedule = hasKnownAmounts && milestones.length >= 2;

  const scheduleRowsHtml = milestones
    .map(
      (milestone) => `
        <p style="margin: 0 0 6px 0; color: #64748b; font-size: 13px; line-height: 1.5;">${escapeHtml(milestone.label)}${Number(milestone.percent) > 0 ? ` (${escapeHtml(formatPercentLabel(milestone.percent))}%)` : ''} : <strong style="color: #334155;">${escapeHtml(formatCurrency(milestone.amountTTC))}</strong><br>
        <span style="font-size: 11px; color: #94a3b8;">${escapeHtml(milestone.dueLabel || '')}</span></p>`
    )
    .join('');

  const amountCardsHtml = showFullSchedule
    ? `
      <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px 14px; margin: 0 0 14px 0;">
        <p style="margin: 0 0 8px 0; color: #64748b; font-size: 13px; line-height: 1.5;">Montant total TTC : <strong style="color: #0f172a;">${escapeHtml(formatCurrency(totalTTC))}</strong></p>
        <p style="margin: 0 0 8px 0; color: #0f172a; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;">Échéancier de règlement</p>
        ${scheduleRowsHtml}
        <p style="margin: 6px 0 0 0; font-size: 11px; color: #94a3b8;">Échéancier complet repris dans le devis et ses conditions de règlement.</p>
      </div>
    `
    : hasKnownAmounts
      ? `
      <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px 14px; margin: 0 0 14px 0;">
        ...
        <p style="margin: 0 0 6px 0; color: #64748b; font-size: 13px; line-height: 1.5;">Acompte prévu au devis (${escapeHtml(formatPercentLabel(depositPercent))}%) : <strong style="color: #334155;">${escapeHtml(formatCurrency(depositAmount))}</strong></p>
        <p style="margin: 0; font-size: 11px; color: #94a3b8;">Les échéances suivantes figurent dans le devis et les conditions de règlement.</p>
      </div>
    `
      : `
...
// toPdfPoints / topMmToPdfY / resolveAnchorPage : voir lib/pdf-signature-anchors.mjs
// (module pur importé ci-dessus, testé indépendamment).
...
const uploadPdfBuffer = async (path, buffer, contentType = PDF_CONTENT_TYPE) => {
...
// Destinataires d'une session : le client, puis les adresses supplémentaires
// saisies à l'envoi (même liste pour les relances et la confirmation).
const getRecipientEmails = (session) =>
  [session?.recipient?.email, ...(session?.recipient?.additionalEmails || [])].filter(Boolean);

// Pièces jointes supplémentaires conservées avec la session : rejointes aux
// relances, proposées au téléchargement sur la page de signature.
const downloadExtraAttachments = async (session) =>
  Promise.all(
    (session?.document?.extraAttachments || [])
      .filter((entry) => entry?.path)
      .map(async (entry) => ({
        filename: entry.filename,
        content: await downloadPdfBuffer(entry.path),
        contentType: entry.contentType || 'application/octet-stream',
      }))
  );

/* -------------------------------------------------------------------------- */
/*  Uploads en morceaux (staging) — contournement de la limite ~4,5 Mo/requête */
/* -------------------------------------------------------------------------- */

// Le chemin est TOUJOURS préfixé par l'uid authentifié : un utilisateur ne peut
// ni lire ni écraser l'upload d'un autre, quel que soit l'uploadId fourni.
const buildStagedChunkPath = (userId, uploadId, chunkIndex) =>
  `${STAGING_ROOT}/${userId}/${uploadId}/part-${String(chunkIndex).padStart(5, '0')}`;

export const stageQuoteDeliveryPdfChunk = async ({
  userId,
  uploadId,
  chunkIndex,
  totalChunks,
  ...
  if (!STAGED_UPLOAD_ID_PATTERN.test(String(uploadId || ''))) {
    throw createHttpError("Identifiant d'upload invalide.", 400);
  }

  const total = Number(totalChunks);
  const index = Number(chunkIndex);

  if (!Number.isInteger(total) || total < 1 || total > STAGED_UPLOAD_MAX_CHUNKS) {
    throw createHttpError('Découpage du PDF invalide.', 400);
  }

  if (!Number.isInteger(index) || index < 0 || index >= total) {
    throw createHttpError('Morceau de PDF invalide.', 400);
  }

  if (typeof chunkBase64 !== 'string' || !chunkBase64.trim()) {
    throw createHttpError('Morceau de PDF manquant.', 400);
  }
  ...
  if (buffer.length > STAGED_CHUNK_MAX_BYTES) {
    throw createHttpError('Morceau de PDF trop volumineux.', 413);
  }

  const bucket = getFirebaseAdminStorage().bucket();
  await bucket.file(buildStagedChunkPath(userId, uploadId, index)).save(buffer, {
    resumable: false,
    metadata: {
      contentType: 'application/octet-stream',
      cacheControl: 'private, max-age=0, no-store',
      metadata: { totalChunks: String(total) },
    },
  });

  return { ok: true, uploadId, chunkIndex: index, totalChunks: total };
};

// Réassemble un PDF téléversé en morceaux. Vérifie l'exhaustivité (aucun trou),
// la taille totale et la signature %PDF avant de rendre le buffer.
const readStagedQuoteDeliveryFile = async (userId, uploadId, { requirePdf = true } = {}) => {
  if (!STAGED_UPLOAD_ID_PATTERN.test(String(uploadId || ''))) {
    throw createHttpError('Le PDF source est manquant.', 400);
  }

  const bucket = getFirebaseAdminStorage().bucket();
  const prefix = `${STAGING_ROOT}/${userId}/${uploadId}/`;
  const [files] = await bucket.getFiles({ prefix });

  if (!files.length) {
    throw createHttpError("Le PDF téléversé est introuvable. Relancez l'envoi du devis.", 400);
  }

  const sortedFiles = files.slice().sort((a, b) => a.name.localeCompare(b.name));
  const expectedTotal = Number(sortedFiles[0]?.metadata?.metadata?.totalChunks || 0);
  const isComplete =
    Number.isInteger(expectedTotal) &&
    expectedTotal >= 1 &&
    sortedFiles.length === expectedTotal &&
    sortedFiles.every((file, index) => file.name === buildStagedChunkPath(userId, uploadId, index));

  if (!isComplete) {
    throw createHttpError("Le PDF téléversé est incomplet. Relancez l'envoi du devis.", 400);
  }
  ...
  if (pdfBuffer.length > STAGED_PDF_MAX_BYTES) {
    throw createHttpError(
      'Le PDF du devis dépasse 25 Mo : réduisez le poids des photos avant envoi.',
      413
    );
  }

  if (requirePdf && pdfBuffer.subarray(0, 5).toString('latin1') !== '%PDF-') {
    throw createHttpError('Le PDF source est invalide.', 400);
  }
...
const readStagedQuoteDeliveryPdf = (userId, uploadId) =>
  readStagedQuoteDeliveryFile(userId, uploadId, { requirePdf: true });

// Nettoyage best-effort après envoi : supprime les uploads consommés et purge
// les restes de plus de 24 h (envois abandonnés). Ne fait jamais échouer l'envoi.
const cleanupStagedQuoteDeliveryUploads = async (userId, uploadIds) => {
  const bucket = getFirebaseAdminStorage().bucket();

  for (const uploadId of (uploadIds || []).filter(Boolean)) {
    try {
      await bucket.deleteFiles({ prefix: `${STAGING_ROOT}/${userId}/${uploadId}/`, force: true });
    } catch (error) {
      console.warn('[cleanupStagedQuoteDeliveryUploads] Upload non purgé', {
        uploadId,
  ...
  try {
    const [files] = await bucket.getFiles({ prefix: `${STAGING_ROOT}/${userId}/` });
    ...
    for (const file of files) {
      ...
      if (createdAt && createdAt < cutoff) {
        try {
          ...
          // Ignoré : la purge repassera au prochain envoi.
        }
      }
    }
  } catch (error) {
    console.warn('[cleanupStagedQuoteDeliveryUploads] Purge des uploads périmés impossible', {
      message: error?.message,
    });
  }
};
...
const buildQuoteModel = (quoteData, pdfInfo) => {
  ...
  const payment = buildPaymentModel(quoteData, totalTTC, pdfInfo?.paymentMilestones);
...
const buildSessionPayload = ({
  ...
  return {
    ...
    recipient,
    quote,
    // Objet et message personnels (résolus ensuite contre le gabarit par
    // applyCustomEmailTexts : null = gabarit inchangé).
    customSubject: normalizeEnv(customSubject) || null,
    customMessage: typeof customMessage === 'string' && customMessage.trim() ? customMessage : null,
    ...
    document: {
      ...
      // Pièces jointes supplémentaires (PDF, JPG, PNG) conservées avec la session.
      extraAttachments: Array.isArray(extraAttachments)
        ? extraAttachments
        : existingSession?.document?.extraAttachments || [],
    },
    signature: {
  ...
  return getResponsiveEmailLayout({
    ...
    contentHtml: `
      ...
      <tr>
        ...
        ? customMessageToHtml(session.customMessage)
...
const buildSignedConfirmationHtml = ({ session }) => {
  ...
  const step1Text = hasMeasurementVisit
    ? "Pour lancer officiellement votre commande, merci de procéder au règlement de l'acompte. Celui-ci peut être réglé par virement à la commande ou, lors de la prise de côtes, par chèque ou virement."
...
const buildDefaultDeliveryEmailSubject = (session) =>
...
const buildDeliveryEmailSubject = (session) =>
  session.customSubject || buildDefaultDeliveryEmailSubject(session);

// Introduction du gabarit, en texte : proposée telle quelle à la modification
// dans la fenêtre de vérification.
const buildDefaultDeliveryMessage = (session) =>
  [
    `Veuillez trouver ci-joint votre devis n°${getDisplayQuoteNumber(session.quote?.number)} concernant votre projet de menuiseries.`,
    'En tant que fabricant direct, notre priorité chez SARANGE est de vous proposer des menuiseries de haute qualité, sans intermédiaire, et toujours au juste prix.',
  ].join('\n\n');

// Objet et message saisis : conservés seulement s'ils diffèrent du gabarit.
const applyCustomEmailTexts = (session, { customSubject, customMessage } = {}) => ({
  ...session,
  customSubject: resolveCustomSubject(customSubject, buildDefaultDeliveryEmailSubject(session)),
  customMessage: resolveCustomMessage(customMessage, buildDefaultDeliveryMessage(session)),
});
...
const toPublicSessionResponse = (session, { active = true } = {}) => {
  ...
  return {
    ...
    // Pièces jointes supplémentaires de l'envoi (téléchargeables par le client).
    attachments: (session.document?.extraAttachments || []).map((entry, index) => ({
      index,
      filename: entry.filename || `piece-jointe-${index + 1}`,
      size: Number(entry.size || 0),
      contentType: entry.contentType || '',
      url: buildSignatureAttachmentHref(session.id, index),
    })),
...
/**
 * ─────────────────────────────────────────────────────────────────────────
 * ATTENTION — bug récurrent déjà corrigé deux fois : un élément rattaché à un
 * repère (`signatureAnchors.xxx`) dessiné sur la mauvaise page. Un devis
 * répartit son contenu sur un nombre de pages VARIABLE (nombre de lignes,
 * échéancier, CGV) : la case de certification TVA réduite, un tampon de
 * panneau, ou tout futur repère peuvent très bien vivre sur une page
 * DIFFÉRENTE de celle du bloc « Bon pour accord ».
 *
 * `signaturePage`/`signaturePageHeight` ci-dessous ne concernent QUE ce
 * bloc « Bon pour accord » (mention, date, cadre de signature) : ils
 * partagent tous le même `signatureAnchors.pageNumber` car ils sont capturés
 * ensemble dans `drawSignatureSection` (lib/pdf-generator.js). Pour TOUT
 * AUTRE repère, ne jamais réutiliser `signaturePage` directement — passer
 * par `drawAtAnchor` (lib/pdf-signature-anchors.mjs), qui résout la page
 * PROPRE au repère et rend l'erreur impossible plutôt que de compter sur la
 * vigilance de qui relit ce fichier.
 * ─────────────────────────────────────────────────────────────────────────
 */
const applySignatureToPdf = async ({
  ...
  // Page du bloc « Bon pour accord » UNIQUEMENT — voir l'avertissement ci-dessus.
  const pageIndex = Math.max(0, Number(signatureAnchors.pageNumber) - 1);
  const signaturePage = pdfDocument.getPage(pageIndex);
  const signaturePageHeight = signaturePage.getHeight();

  signaturePage.drawText('Lu et approuve, bon pour accord', {
    x: toPdfPoints(signatureAnchors.agreementMention?.x || 0),
    y: topMmToPdfY(signaturePageHeight, signatureAnchors.agreementMention?.y || 0),
    size: 11,
    font: italicFont,
    color: rgb(0.1, 0.13, 0.2),
  });

  signaturePage.drawText(formatDateLabel(signedAt), {
    x: toPdfPoints(signatureAnchors.dateLine?.valueX || 0),
    y: topMmToPdfY(signaturePageHeight, signatureAnchors.dateLine?.valueY || 0),
    size: 10,
    font: italicFont,
    color: rgb(0.1, 0.13, 0.2),
  });
  ...
  signaturePage.drawImage(embeddedImage, {
    x: imageX,
    y: imageY,
    width: imageWidth,
    height: imageHeight,
  ...
  if (signerName) {
    signaturePage.drawText(String(signerName).trim(), {
  ...
  // La case à cocher TVA réduite peut se trouver sur une AUTRE page que la
  // zone de signature (échéancier ou CGV plus longs selon le devis) : on
  // passe TOUJOURS par drawAtAnchor, qui résout et dessine sur SA page —
  // jamais sur `signaturePage` (voir l'avertissement en tête de fonction).
  if (acceptReducedVat && signatureAnchors.reducedVatCheckbox) {
    const checkbox = signatureAnchors.reducedVatCheckbox;
    drawAtAnchor(pdfDocument, checkbox, signaturePage, (checkboxPage, checkboxPageHeight) => {
      const segments = buildCheckmarkSegments(checkboxPageHeight, checkbox);
      segments.forEach((segment) => {
        checkboxPage.drawLine({ ...segment, thickness: 1.2, color: rgb(0.1, 0.13, 0.2) });
      });
    });
  }
  ...
  if (Array.isArray(panelStampEntries) && panelStampEntries.length > 0) {
    try {
      ...
      // 1) Tampon inline, à la position exacte de la ligne porte. Repère par
      // ligne : chacun porte SON pageNumber (l'échéancier/les CGV peuvent
      // pousser des lignes sur des pages différentes) — drawAtAnchor est le
      // seul point de résolution, pas de fallback ici (pas de page "à
      // défaut" sensée pour un tampon de ligne : on omet plutôt que risquer
      // le mauvais endroit).
      panelStampEntries.forEach((entry) => {
        const anchor = anchorByLine.get(entry.lineId);
        drawAtAnchor(pdfDocument, anchor, null, (target, targetHeight) => {
...
export const createAndSendQuoteDelivery = async ({
  ...
  // Garde serveur : aucun envoi (et donc aucune signature) tant que le type de
  // client n'a pas été EXPLICITEMENT choisi — jamais déduit du SIRET, du nom
  // ou de l'adresse. Les CGV du PDF en dépendent.
  if (!isKnownClientType(quoteData?.payload?.clientData?.clientType)) {
    throw createHttpError(
      'Type de client à confirmer : choisissez Particulier ou Professionnel sur le devis avant son envoi.',
      400
    );
  }

  // Régime fiscal : un devis en autoliquidation BTP ne part pas sans l'identité
  // fiscale complète du donneur d'ordre, et un taux 0 % hors sous-traitance est
  // refusé — mêmes règles que le récapitulatif, appliquées côté serveur.
  const quoteClientData = quoteData?.payload?.clientData;
  const validatedVariants =
    quoteData?.payload?.variantsMode === true && Array.isArray(quoteData?.payload?.variants)
      ? quoteData.payload.variants
      : [{ cartItems: quoteData?.payload?.cartItems, tvaRate: quoteData?.payload?.tvaRate }];

  validatedVariants.forEach((variant) => {
    const taxValidation = getTaxRegimeValidation({
      clientType: quoteClientData?.clientType,
    ...
    if (!taxValidation.isValid) {
      throw createHttpError(taxValidation.errors[0], 400);
    }
  });
  ...
  // Deux canaux d'arrivée du PDF : `pdfUploadId` (téléversé en morceaux, seul
  // canal viable pour les devis lourds/multi-variantes) ou `pdfBase64` inline
  // (conservé pour les clients dont le JS en cache date d'avant ce mécanisme).
  const usedUploadIds = [];
  let pdfBuffer;
  if (normalizeEnv(pdfUploadId)) {
    pdfBuffer = await readStagedQuoteDeliveryPdf(userId, pdfUploadId);
    usedUploadIds.push(pdfUploadId);
  } else {
    pdfBuffer = parsePdfBuffer(pdfBase64);
  }
  const now = new Date();
  ...
  if (Array.isArray(variants) && variants.length > 1) {
    ...
    for (let index = 0; index < variants.length; index += 1) {
      ...
      let variantBuffer;
      if (normalizeEnv(variant?.pdfUploadId)) {
        variantBuffer = await readStagedQuoteDeliveryPdf(userId, variant.pdfUploadId);
        usedUploadIds.push(variant.pdfUploadId);
      } else {
        variantBuffer = parsePdfBuffer(variant?.pdfBase64);
      }
      ...
      documentVariants.push({
        ...
        // Échéancier complet de la variante, tel qu'affiché sur son PDF.
        paymentMilestones: sanitizePaymentMilestones(variant?.paymentMilestones),
  ...
  // Pièces jointes supplémentaires (PDF, JPG, PNG) : type vérifié sur les
  // octets, limites de nombre et de poids, conservées avec la session pour
  // les relances et la page de signature.
  const requestedAttachments = Array.isArray(extraAttachments) ? extraAttachments : [];
  if (requestedAttachments.length > ATTACHMENT_LIMITS.maxFiles) {
    throw createHttpError(`Au plus ${ATTACHMENT_LIMITS.maxFiles} pièces jointes par envoi.`, 400);
  }
  const storedAttachments = [];
  const attachmentPayloads = [];
  let attachmentsBytes = pdfBuffer.length;
  for (const [index, entry] of requestedAttachments.entries()) {
    const label = normalizeEnv(entry?.filename) || `n°${index + 1}`;
    const buffer = await readStagedQuoteDeliveryFile(userId, entry?.uploadId, { requirePdf: false });
    usedUploadIds.push(entry.uploadId);
    const contentType = detectAttachmentType(buffer);
    if (!contentType) {
      throw createHttpError(`La pièce jointe « ${label} » n'est ni un PDF ni une image JPG/PNG.`, 400);
    }
    if (buffer.length > ATTACHMENT_LIMITS.maxFileBytes) {
      throw createHttpError(`La pièce jointe « ${label} » dépasse 8 Mo.`, 413);
    }
    attachmentsBytes += buffer.length;
    if (attachmentsBytes > ATTACHMENT_LIMITS.maxTotalBytes) {
      throw createHttpError('Devis compris, les pièces jointes dépassent 15 Mo au total.', 413);
    }
    const filename = normalizeAttachmentFilename(entry?.filename, contentType);
    const storagePath = `${STORAGE_ROOT}/${sessionId}/attachment-${index + 1}/${normalizeFilename(
      filename,
      `piece-jointe-${index + 1}`
    )}`;
    await uploadPdfBuffer(storagePath, buffer, contentType);
    storedAttachments.push({ filename, path: storagePath, size: buffer.length, contentType });
    attachmentPayloads.push({ filename, content: buffer, contentType });
  }

  const sessionPayload = applyCustomEmailTexts(
    buildSessionPayload({
      existingSession,
      sessionId,
      userId,
      ...
      originalFilename,
      variants: documentVariants,
      extraAttachments: storedAttachments,
      customSubject,
      customMessage,
      now,
    }),
  ...
  // Destinataires saisis dans la fenêtre de vérification : le premier devient
  // le client de la session, les autres reçoivent les mêmes e-mails.
  const recipientList = parseRecipientEmails(recipients);
  if (recipientList.length) {
    const recipientProblem = validateRecipientEmails(recipientList);
    if (recipientProblem) {
      throw createHttpError(recipientProblem, 400);
    }
    sessionPayload.recipient = {
      ...sessionPayload.recipient,
      email: recipientList[0],
      additionalEmails: recipientList.slice(1),
    };
  }
  ...
  try {
    const deliveryInfo = await sendMail({
      to: getRecipientEmails(sessionPayload),
    ...
    await cleanupStagedQuoteDeliveryUploads(userId, usedUploadIds);
...
/* ─── Aperçus AVANT envoi (vérification par l'utilisateur) ───────────────── */
// Mêmes contrôles, mêmes gabarits et même destinataire que l'envoi réel, mais
// rien n'est enregistré ni envoyé : la modale de confirmation affiche
// exactement ce qui partira.

const PREVIEW_SESSION_ID = 'apercu';
const PREVIEW_LINK_PLACEHOLDER = '[lien de signature personnel, créé à l’envoi]';

const buildPreviewDocumentVariants = (variants) =>
  Array.isArray(variants) && variants.length > 1
    ? variants.map((variant, index) => ({
        ...
        panelSelections: Array.isArray(variant?.panelSelections) ? variant.panelSelections : [],
        pdfPath: null,
        filename: normalizeFilename(variant?.filename || `variante-${index + 1}.pdf`),
        signatureAnchors: variant?.signatureAnchors || null,
      }))
    : null;

export const previewQuoteDelivery = async ({
  userId,
  quoteId,
  deliveryMode,
  pdfInfo,
  ...
  const db = getFirebaseAdminDb();
  const quoteSnapshot = await readQuoteDocumentRef(db, userId, quoteId).get();
  if (!quoteSnapshot.exists) {
    throw createHttpError('Devis introuvable.', 404);
  }
  const quoteData = quoteSnapshot.data();
  ...
  if (existingSessionId) {
    const existingSnapshot = await readSessionDocumentRef(db, existingSessionId).get();
    existingSession = existingSnapshot.exists
  ...
  const session = applyCustomEmailTexts(
    buildSessionPayload({
      existingSession,
      sessionId: existingSession?.id || PREVIEW_SESSION_ID,
      userId,
      quoteId,
      deliveryMode,
      quoteData,
      pdfInfo,
      originalPdfPath: existingSession?.document?.originalPdfPath || null,
      originalFilename,
      variants: buildPreviewDocumentVariants(variants),
      extraAttachments: [],
      now,
    }),
    { customSubject, customMessage }
  ...
  const recipientList = parseRecipientEmails(recipients);
  if (!session.recipient.email && !recipientList.length) {
    throw createHttpError("Aucune adresse email client n'est renseignée sur ce devis.", 400);
  }

  const link = buildSignatureLink(session);
  const defaultMessage = buildDefaultDeliveryMessage(session);
  let text = htmlToPlainText(buildDeliveryEmailHtml({ session }));
  // Même mail avec un repère à la place du message : la fenêtre de
  // vérification y insère le texte saisi au fil de la frappe.
  let textTemplate = htmlToPlainText(
    buildDeliveryEmailHtml({ session: { ...session, customMessage: CUSTOM_MESSAGE_PLACEHOLDER } })
  );
  if (isNewSession && link) {
    text = text.split(link).join(PREVIEW_LINK_PLACEHOLDER);
    textTemplate = textTemplate.split(link).join(PREVIEW_LINK_PLACEHOLDER);
  }

  return {
    kind: 'quote-delivery',
    deliveryMode,
    to: recipientList.length ? formatRecipientEmails(recipientList) : session.recipient.email,
    recipientName: session.recipient.fullName || '',
    subject: buildDeliveryEmailSubject(session),
    defaultSubject: buildDefaultDeliveryEmailSubject(session),
    defaultMessage,
    message: session.customMessage || defaultMessage,
    text,
    textTemplate,
    attachments: [
      { filename: originalFilename },
      ...(Array.isArray(extraAttachments) ? extraAttachments : []).map((entry) => ({
        filename: normalizeEnv(entry?.filename) || 'piece-jointe',
        size: Number(entry?.size || 0),
      })),
    ],
    link: isNewSession ? '' : link,
    linkIsNew: isNewSession && Boolean(link),
    expiresAt: timestampToIso(session.expiresAt),
    quoteNumber: session.quote?.number || '',
  };
};

export const previewQuoteSignatureReminder = async ({ sessionId, reminderLevel }) => {
  ensureServerConfig({
    requireEmail: true,
  ...
  if (session.deliveryMode !== 'signature') {
    throw createHttpError("Ce devis n'est pas configure pour la signature en ligne.", 409);
  }
  const status = normalizeQuoteSignatureStatus(session.status);
  if (status === 'signed') {
    throw createHttpError('Ce devis est déjà signé.', 409);
  }
  if (status === 'refused') {
  ...
  const reminder = buildReminderEmailHtml({ session, level: reminderLevel });

  return {
    kind: 'quote-reminder',
    level: reminder.level,
    to: formatRecipientEmails(getRecipientEmails(session)),
    recipientName: session.recipient?.fullName || '',
    subject: reminder.subject,
    text: htmlToPlainText(reminder.html),
    attachments: [
      ...(session.document?.originalFilename ? [{ filename: session.document.originalFilename }] : []),
      ...(session.document?.extraAttachments || []).map((entry) => ({
        filename: entry.filename,
        size: Number(entry.size || 0),
      })),
    ],
    link: buildSignatureLink(session),
    linkIsNew: false,
    expiresAt: timestampToIso(session.expiresAt),
    quoteNumber: session.quote?.number || '',
  };
...
export const sendQuoteSignatureReminder = async ({ sessionId, reminderLevel }) => {
  ...
  await sendMail({
    to: getRecipientEmails(context.session),
    subject: reminder.subject,
    html: reminder.html,
    attachments: [
      {
      ...
      ...(await downloadExtraAttachments(context.session)),
    ],
  });
...
/**
 * Aperçu d'un lien de signature pour les métadonnées de partage (WhatsApp,
 * SMS, e-mail). Lecture SANS effet de bord : contrairement à
 * getQuoteSignatureSession, la session n'est jamais marquée « consultée »
 * (le robot d'aperçu n'est pas le client) ni expirée en base.
 */
export const getQuoteSignatureLinkPreview = async (sessionId) => {
  const { session } = await loadSessionContext(sessionId);
  const status = normalizeQuoteSignatureStatus(session.status);
  const expiresAt = timestampToIso(session.expiresAt);
  const expiresAtMs = expiresAt ? new Date(expiresAt).getTime() : NaN;
  const pastExpiry = Number.isFinite(expiresAtMs) && expiresAtMs < Date.now();

  return {
    deliveryMode: session.deliveryMode || 'signature',
    status: pastExpiry && ['sent', 'viewed'].includes(status) ? 'expired' : status,
    quoteNumber: session.quote?.number || '',
    quoteTitle: session.quote?.title || '',
    totalTTC: Number(session.quote?.totalTTC || 0),
    recipientName: session.recipient?.fullName || '',
    variantsMode: session.variantsMode === true,
    expiresAt,
...
/**
 * Pièce jointe supplémentaire d'un envoi, servie sur la page de signature.
 */
export const getQuoteSignatureAttachment = async (sessionId, index) => {
  const { session } = await loadSessionContext(sessionId);
  const entries = Array.isArray(session.document?.extraAttachments) ? session.document.extraAttachments : [];
  const entry = Number.isInteger(index) && index >= 0 ? entries[index] : null;
  if (!entry?.path) {
    throw createHttpError('Pièce jointe introuvable.', 404);
  }

  return {
    buffer: await downloadPdfBuffer(entry.path),
    filename: entry.filename || `piece-jointe-${index + 1}`,
    contentType: entry.contentType || 'application/octet-stream',
  };
};
...
export const signQuoteSignatureSession = async ({
  ...
  const signedQuote = variantsMode
    ? {
        ...
        payment: buildPaymentModel(
          ...
          chosenVariant.paymentMilestones
        ),
      }
    : context.session.quote;
  ...
  // Événement financier vers sarange-finances (acompte à facturer) —
  // best-effort, jamais bloquant pour la signature ni pour les e-mails.
  await emitQuoteSignedEvent(getFirebaseAdminDb(), signedSession);

  try {
    await sendMail({
      to: getRecipientEmails(signedSession),
