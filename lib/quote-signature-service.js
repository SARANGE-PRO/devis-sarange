...
const applySignatureToPdf = async ({
  ...
  if (acceptReducedVat && signatureAnchors.reducedVatCheckbox) {
    ...
    // dessine la croix sur SA page, jamais sur celle de la signature.
    const checkboxPage = resolveAnchorPage(pdfDocument, checkbox.pageNumber, page);
    const segments = buildCheckmarkSegments(checkboxPage.getHeight(), checkbox);
