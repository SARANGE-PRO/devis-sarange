...
// Placement des 15 bits de format, IDENTIQUE à l'implémentation de
// référence de Nayuki (bit 0 = poids faible) : toute variation d'ordre rend
// le code illisible.
const placeFormatBits = (modules, size, maskId) => {
  ...
  modules[8][8] = bit(7);
  modules[7][8] = bit(8);
  for (let i = 9; i < 15; i += 1) modules[14 - i][8] = bit(i);

  // Seconde copie, le long des motifs bas-gauche et haut-droit.
  for (let i = 0; i < 8; i += 1) modules[size - 1 - i][8] = bit(i);
  for (let i = 8; i < 15; i += 1) modules[8][size - 15 + i] = bit(i);
