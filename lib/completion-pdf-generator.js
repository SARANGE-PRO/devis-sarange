...
const drawContinuationHeaders = (doc, { pageWidth, fontFamily, completionNumber }) => {
  ...
  for (let pageNumber = 2; pageNumber <= pageCount; pageNumber += 1) {
    ...
    drawText(
      doc,
      `Bon de fin de chantier${completionNumber ? ` — n° ${completionNumber}` : ''}`,
...
const drawReceptionSection = (
  ...
  // Garanties et effets de la réception : LISTE À PUCES ORANGES dans une
  // carte blanche, même langage que les cartes « Conditions de règlement » /
  // « Garanties » du devis.
  const bullets = [
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...COLORS.slate500);
  drawText(doc, 'GARANTIES — EFFETS DE LA RÉCEPTION', PAGE_MARGIN + 4, y + 7);
...
export const buildCompletionPdfDocument = async ({
  ...
  drawContinuationHeaders(doc, { pageWidth, fontFamily, completionNumber });
...
export const buildReservesLiftPdfDocument = async ({
  ...
  drawContinuationHeaders(doc, { pageWidth, fontFamily, completionNumber: liftNumber });
  drawFooterOnAllPages(doc, { pageWidth, pageHeight, fontFamily, footerLogoDataUrl });
