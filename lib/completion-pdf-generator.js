...
/**
 * Textes du document selon son type. AVEC POSE → PV de réception des travaux
 * (art. 1792-6, garanties travaux). SANS POSE → bon d'enlèvement ou de
 * livraison : constat de remise des produits, garanties de conformité /
 * vices cachés / fabricant et transfert des risques — jamais de garanties
 * travaux ni de retenue (loi de 1971 : marchés de travaux uniquement).
 */
const getDocTypeTexts = (docType = 'reception') => {
  ...
  return {
    ...
    lead: "Le client, maître d'ouvrage, après visite contradictoire des ouvrages désignés ci-dessus et réalisés conformément au devis référencé ci-dessus, prononce ce jour la réception des travaux.",
    ...
    guarantees: [
      "Garantie de parfait achèvement (1 an) : couvre les réserves ci-dessus et tout désordre notifié par écrit dans l'année.",
      "Garantie de bon fonctionnement des éléments d'équipement dissociables (2 ans).",
      `Garantie décennale (10 ans), couverte par ${COMPANY.insurance} — attestation sur demande.`,
    ],
    mention: (hasReserves) =>
...
const drawReceptionSection = (
  ...
  if (hasReserves && hasKnownBalance) {
    bullets.push(
      retentionEligible
        ? "Retenue de garantie : conformément à la clause du devis d'origine (art. 4.5 des CGV), 5% du montant total du devis est conservé jusqu'à la levée des réserves, libéré au plus tard un an après la présente réception."
        : "Le devis d'origine ne comporte pas de clause de retenue de garantie : le solde ci-dessous reste dû en totalité malgré les réserves."
    );
  }
...
const drawBalanceSection = (doc, { pageWidth, pageHeight, fontFamily, cursorY, balance, hasReserves, invoiceReference, quoteNumber, texts }) => {
  ...
  const totalLabel =
    hasReserves && balance.retenueGarantie > 0 ? 'SOLDE VERSÉ À RÉCEPTION' : texts.soldeLabel;

  // Hauteur = titre 13 + lignes 6 chacune + bande orange 11 + bloc RIB 20
  // + marge : le texte ne doit JAMAIS déborder sous la bordure de la carte.
  const boxHeight = 13 + rows.length * 6 + 3 + 11 + 4 + 20;
  ...
  drawText(doc, formatCurrency(balance.soldeAPercevoir), PAGE_MARGIN + boxWidth - 8, rowY + 2, { align: 'right' });
  rowY += 11;
 ...
 * points et acte la libération de l'éventuelle retenue de garantie.
 */
export const buildReservesLiftPdfDocument = async ({
  ...
  // Montant restant à percevoir à la levée (retenue libérée, reliquat...),
  // saisi/confirmé par l'utilisateur dans la modale d'envoi. 0 → aucun bloc
  // de règlement sur le document.
  amountDue = 0,
  paymentReference = '',
  issueDate = new Date(),
} = {}) => {
  ...
  const legalLines = [
    ...
    ...(retentionWasApplied
      ? ['La retenue de garantie appliquée lors de la réception est libérée : son montant devient', 'exigible au profit de SARANGE.']
      : []),
  ...
  // levée (retenue de garantie libérée, reliquat impayé...), confirmé par
  // l'utilisateur à l'envoi — jamais un calcul silencieux.
