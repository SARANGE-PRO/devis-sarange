...
const drawHeader = (doc, { pageWidth, fontFamily, title, subtitle, badgeLabel, badgeValue }) => {
  ...
  doc.setFontSize(9.4);
  doc.setTextColor(...COLORS.brand);
  drawText(doc, badgeValue || '(à renseigner)', pageWidth - PAGE_MARGIN - 28, 43.6, { align: 'center' });
  doc.setFont(fontFamily, 'normal');
  doc.setFontSize(7);
  doc.setTextColor(...COLORS.brand);
...
const drawInfoCards = (doc, { pageWidth, fontFamily, cursorY, references, clientLines }) => {
  ...
  const rowHeight = 6;
  const cardHeight = 12 + Math.max(references.length, clientLines.length) * rowHeight;
  ...
  drawOneCard(PAGE_MARGIN, 'Références', references);
  drawOneCard(PAGE_MARGIN + cardWidth + 10, 'Client et chantier', clientLines);
...
const drawWorksTable = (doc, { pageWidth, pageHeight, fontFamily, cursorY, ouvrages }) => {
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...COLORS.slate500);
  drawText(doc, 'OUVRAGES RÉCEPTIONNÉS', PAGE_MARGIN, y);
  y += 6;
  ...
  ouvrages.forEach((item, index) => {
    ...
    const subLines = item.sub
      ? doc.splitTextToSize(sanitizePdfText(item.sub), colDesig - 6)
      : [];
    ...
    drawText(doc, item.repere || '-', PAGE_MARGIN + colDesig + 3, y + 5);
...
const drawReceptionSection = (
  doc,
  { pageWidth, pageHeight, fontFamily, cursorY, hasReserves, reserves, reserveLiftDelayDays, retentionEligible }
) => {
  let y = cursorY;
  doc.setFont(fontFamily, 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(...COLORS.slate500);
  drawText(doc, 'RÉCEPTION DES TRAVAUX', PAGE_MARGIN, y);
  y += 6;
...
const drawBalanceSection = (doc, { pageWidth, pageHeight, fontFamily, cursorY, balance, hasReserves, invoiceReference, quoteNumber }) => {
  ...
  const boxHeight = 12 + rows.length * 6 + 6 + 16;
  y = ensureSpace(doc, y, boxHeight + 4, pageWidth, pageHeight);
  ...
  drawText(
    doc,
    `Merci d'indiquer la référence ${invoiceReference || '(à renseigner)'} en libellé du virement.`,
    PAGE_MARGIN + 4,
    rowY + 14
  );
...
const drawSignatureSection = (doc, { pageWidth, pageHeight, fontFamily, cursorY, hasReserves, issueDateLabel, lieu }) => {
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...COLORS.slate500);
  drawText(doc, 'SIGNATURE', PAGE_MARGIN, y);
  ...
  doc.setFontSize(8);
  doc.setTextColor(...COLORS.slate500);
  drawText(doc, 'SIGNATURE DU CLIENT', signatureBox.x + signatureBox.width / 2, signatureBox.y, {
    align: 'center',
  });
...
const drawFooter = (doc, { pageWidth, pageHeight, fontFamily }) => {
...
export const buildCompletionPdfDocument = ({
  ...
  let cursorY = drawHeader(doc, {
    ...
    badgeLabel: 'Réf. facture',
    badgeValue: invoiceReference,
  });
  ...
  drawFooter(doc, { pageWidth, pageHeight, fontFamily });
  void finalY;
