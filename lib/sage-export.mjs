...
import { LINE_NATURES, resolveLineNature } from './line-nature.mjs';
...
// (art. 283, 2 nonies du CGI — cf. mention légale du PDF). Le régime « exonération »
...
export const SAGE_VAT_REGIMES = Object.freeze([
  ...
  { id: 'tva-20', rate: 20, label: 'TVA 20 %' },
  {
    id: 'autoliquidation',
    rate: 0,
    label: 'Autoliquidation BTP — article 283, 2 nonies du CGI',
  },
...
/* ─── Natures de ligne ───────────────────────────────────────────────────── */
// Source de vérité mutualisée avec la ventilation des échéanciers
// (lib/line-nature.mjs). Le nom SAGE_LINE_NATURES est conservé pour les
// consommateurs existants de ce module.
export const SAGE_LINE_NATURES = LINE_NATURES;
 ...
 * @param {string}  input.commissionMode    'percent' ou 'amount' (quoteSettings)
 * @param {number}  input.commissionPercent commission en % (quoteSettings, si mode percent)
 * @param {number}  input.commissionAmount  commission en € (quoteSettings, si mode amount)
...
export const buildSageExportModel = (input, deps) => {
  ...
  const commissionMode = input?.commissionMode === 'amount' ? 'amount' : 'percent';
  ...
  const items =
    typeof applyCommissionToCartItems === 'function'
      ? applyCommissionToCartItems(rawItems, {
          mode: commissionMode,
