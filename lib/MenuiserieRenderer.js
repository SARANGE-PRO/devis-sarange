...
export class MenuiserieRenderer {
  ...
  drawCasementSash(x, y, w, h, sash, config) {
    ...
    if (sash.handle) {
      const fraction = config.handleHeightFraction ?? 0.5;
      if (isDecoPanel) {
        this.drawDoorHandle(x, y, w, h, sash.handle, fraction);
      } else {
  ...
  drawDoorHandle(sashX, sashY, sashW, sashH, position, fraction = 0.5) {
    ...
    // Centre de la barre de tirage selon la hauteur choisie, bornée à la porte.
    const minY = sashY + margin;
    const maxY = sashY + sashH - barH - margin;
    const barY = Math.max(minY, Math.min(maxY, sashY + sashH * fraction - barH / 2));
