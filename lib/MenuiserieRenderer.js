...
export class MenuiserieRenderer {
  ...
  draw(config) {
    ...
    if (sashes && sashes.length > 0) {
      ...
      this.drawGlass(innerX, innerY, innerW, innerH);
    }
  ...
  drawCasementSash(x, y, w, h, sash, config) {
    ...
    if (isDecoPanel) {
      ...
      if (validSbHeight > 0) {
        ...
        // 1. Vitrage supérieur
        this.drawGlass(glassX, glassY, glassW, topGlassH);
        ...
        this.drawRect(glassX, sbY, glassW, validSbHeight, this.colors.frame, this.colors.frameBorder, 1);
        
        // 4. Moulure de panneau
        const mGap = this.metrics.mGap;
        this.drawRect(glassX + mGap, sbY + mGap, glassW - mGap*2, validSbHeight - mGap*2, null, '#CCCCCC', 1);
      } else {
        // Plein vitrage
        this.drawGlass(glassX, glassY, glassW, glassH);
      }
    }
    ...
    if (sash.hasVentilation && !isDecoPanel) {
