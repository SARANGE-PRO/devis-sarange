...
export class MenuiserieRenderer {
  ...
  draw(config) {
    ...
    if (voletMonobloc) {
      ...
      this.drawMonoblocCoffre(0, 0, width, coffreH, voletMonoblocManoeuvre);
      currentY = coffreH;
    ...
    if (voletMonobloc) {
      this.drawMonoblocApron(innerX, innerY, innerW, innerH);
    }
  }
  ...
  drawMonoblocCoffre(x, y, w, h, manoeuvre) {
    this.drawRect(x, y, w, h, this.colors.frame, this.colors.frameBorder, 1.5);
  ...
  drawMonoblocApron(x, y, w, innerH) {
    const apronH = Math.min(innerH * 0.22, this.metrics.shutterBoxHeight * 1.4);
    if (apronH <= 0) return;

    // Fond des lames (couleur du profilé pour rester cohérent avec la menuiserie)
    this.drawRect(x, y, w, apronH, this.colors.frame, this.colors.frameBorder, 1);
