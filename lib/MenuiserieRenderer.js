 ...
 */
import { getArchInsetGeometry, getArchSegmentGeometry } from './cintrage.mjs';
...
export class MenuiserieRenderer {
  constructor(canvas) {
    ...
    this.scaleFactor = 1;
    this.metrics = {};
    // Menuiserie cintrée en cours de rendu (null sinon), cf. drawArchedFrame.
    this.arch = null;
    // Décalage du contour cintré appliqué au vitrage des vantaux (cintre intégré).
    this.archGlassInset = null;
  }
  ...
  draw(config) {
    ...
    // --- RENDU SPÉCIFIQUE : MENUISERIE CINTRÉE (Formes spéciales) ---
    if (config.cintrage && type !== 'coulissant') {
      this.drawArchedFrame(config);
      return;
    }
    ...
    // Volet roulant monobloc intégré : on dessine le caisson en partie haute
    // (au-dessus du cadre) puis on décale la menuiserie vers le bas.
    if (voletMonobloc) {
      const coffreH = this.metrics.shutterBoxHeight;
      this.drawMonoblocCoffre(0, 0, width, coffreH, voletMonoblocManoeuvre, voletMonoblocColor);
      currentY = coffreH;
      availableHeight = height - coffreH;
    }
    ...
    // 2. Vantaux
    this.drawSashRow(innerX, innerY, innerW, innerH, sashes, config);

    // 3. Symboles globaux
    if (globalSymbols) {
      const fraction = config.handleHeightFraction ?? 0.5;
      globalSymbols.forEach(symbol => {
        this.drawSymbol(innerX, innerY, innerW, innerH, symbol, fraction);
      });
    }

    // 4. Volet monobloc : premières lames du tablier visibles en haut du vitrage
    if (voletMonobloc) {
      this.drawMonoblocApron(innerX, innerY, innerW, innerH, voletMonoblocColor);
    }
  }

  // Rangée de vantaux (ou vitrage seul) dans la zone intérieure du dormant.
  drawSashRow(innerX, innerY, innerW, innerH, sashes, config) {
    const type = config.type || 'frappe';
  ...
  // ── Menuiserie cintrée (catégorie « Formes spéciales ») ────────────────────
  // Repère : sommet de l'arc en y = 0, naissance du cintre en y = flèche,
  // centre du cercle en (largeur / 2, rayon). Géométrie : lib/cintrage.mjs.

  // Trace le contour cintré complet (arc + montants + traverse basse) décalé
  // de `inset` vers l'intérieur. Ne remplit ni ne trace : l'appelant décide.
  traceArchOutline(inset, bottomY) {
    const geometry = getArchInsetGeometry(this.arch, inset);
    const ctx = this.ctx;
    ctx.beginPath();
    ctx.moveTo(geometry.left, geometry.yJoin);
    ctx.arc(geometry.cx, geometry.cy, geometry.r, geometry.startAngle, geometry.endAngle, false);
    ctx.lineTo(geometry.right, bottomY);
    ctx.lineTo(geometry.left, bottomY);
    ctx.closePath();
  }

  // Segment de cercle (fixe cintré en imposte) : arc décalé de `inset`, fermé
  // par la corde horizontale y = chordY.
  traceArchSegment(inset, chordY) {
    const geometry = getArchSegmentGeometry(this.arch, inset, chordY);
    const ctx = this.ctx;
    ctx.beginPath();
    ctx.moveTo(geometry.left, geometry.chordY);
    ctx.arc(geometry.cx, geometry.cy, geometry.r, geometry.startAngle, geometry.endAngle, false);
    ctx.closePath();
  }

  fillAndStrokePath(fill, stroke, lineWidth = 1) {
    if (fill) {
  ...
  drawArchedFrame(config) {
    const { width, height, sashes, cintrage } = config;
    this.arch = {
      widthMm: width,
      heightMm: height,
      radiusMm: cintrage.radiusMm,
      flecheMm: cintrage.flecheMm,
    };
    const t = this.metrics.outerFrameThick;

    // 1. Dormant : contour cintré plein (couleur du profilé)
    this.traceArchOutline(0, height);
    this.fillAndStrokePath(this.colors.frame, this.colors.frameBorder, 1);

    if (cintrage.type === 'imposte') {
      // 2a. Fixe cintré en imposte : vitrage en segment de cercle sous l'arc,
      // fermé par la traverse à la naissance du cintre.
      const springY = cintrage.flecheMm;
      this.traceArchSegment(t, springY);
      this.fillAndStrokePath(this.colors.glass, this.colors.glassBorder, 1);

      const traverseThick = this.metrics.traverseThick;
      this.drawRect(t, springY, width - t * 2, traverseThick, this.colors.frame, this.colors.frameBorder, 1);

      // 3a. Partie basse : vantaux standards (rectangulaires) sous la traverse.
      const innerY = springY + traverseThick;
      this.drawSashRow(t, innerY, width - t * 2, height - innerY - t, sashes, config);
      return;
    }

    // 2b. Cintre intégré : les vantaux épousent l'arc. Ils sont dessinés
    // rectangulaires sur toute la hauteur puis découpés par le contour
    // intérieur du dormant ; le vitrage est découpé par un contour plus
    // rentré (profilé de vantail) pour faire apparaître la traverse haute cintrée.
    const gap = this.metrics.gap;
    const glassInset = t + gap + this.metrics.sashFrameThick;
    this.archGlassInset = glassInset;
    this.ctx.save();
    this.traceArchOutline(t, height - t);
    this.ctx.clip();
    this.drawSashRow(t, t, width - t * 2, height - t * 2, sashes, config);
    this.ctx.restore();
    this.archGlassInset = null;

    // Arêtes cintrées : dormant, vantail et vitrage
    this.traceArchOutline(t, height - t);
    this.fillAndStrokePath(null, this.colors.frameBorder, 1);
    this.traceArchOutline(t + gap, height - t - gap);
    this.fillAndStrokePath(null, this.colors.frameBorder, 0.5);
    this.traceArchOutline(glassInset, height - glassInset);
    this.fillAndStrokePath(null, this.colors.glassBorder, 1);
  }

  // Exécute `drawFilling` découpé par le contour cintré du vitrage (cintre
  // intégré) ; sans effet sur une menuiserie rectangulaire.
  withArchGlassClip(drawFilling) {
    if (this.archGlassInset == null || !this.arch) {
      drawFilling();
      return;
    }
    this.ctx.save();
    this.traceArchOutline(this.archGlassInset, this.arch.heightMm - this.archGlassInset);
    this.ctx.clip();
    drawFilling();
    this.ctx.restore();
  }

  // Caisson du volet roulant monobloc, dessiné au-dessus du cadre dormant.
  // `color` : teinte propre du volet (coloris choisi), sinon celle du profilé.
  drawMonoblocCoffre(x, y, w, h, manoeuvre, color = null) {
    this.drawRect(x, y, w, h, color || this.colors.frame, this.colors.frameBorder, 1.5);

    // Rainure horizontale décorative (façade du coffre)
    this.ctx.beginPath();
    this.ctx.strokeStyle = this.colors.frameBorder;
    this.ctx.lineWidth = 0.8 * this.scaleFactor;
    const grooveY = y + h * 0.6;
    this.ctx.moveTo(x, grooveY);
    this.ctx.lineTo(x + w, grooveY);
    this.ctx.stroke();

    // Petit panneau photovoltaïque si manœuvre solaire
    if (manoeuvre === 'solaire') {
      const spW = Math.min(w * 0.18, 60 * this.scaleFactor);
      const spH = h * 0.4;
      const spX = x + w - spW - 12 * this.scaleFactor;
      const spY = y + h * 0.3;
      this.drawRect(spX, spY, spW, spH, '#1f2937', '#0f172a', 1);
    }
  }

  // Premières lames du tablier descendues sur le haut du vitrage.
  drawMonoblocApron(x, y, w, innerH, color = null) {
    const apronH = Math.min(innerH * 0.22, this.metrics.shutterBoxHeight * 1.4);
    if (apronH <= 0) return;

    // Fond des lames : coloris du volet s'il est propre, sinon celui du profilé.
    this.drawRect(x, y, w, apronH, color || this.colors.frame, this.colors.frameBorder, 1);

    // Lignes de séparation des lames
    this.ctx.beginPath();
    this.ctx.strokeStyle = '#A8A8A8';
    this.ctx.lineWidth = 0.8 * this.scaleFactor;
    for (let i = y + this.metrics.slatHeight; i < y + apronH; i += this.metrics.slatHeight) {
      this.ctx.moveTo(x, i);
    ...
    this.ctx.beginPath();
    this.ctx.strokeStyle = this.colors.frameBorder;
    this.ctx.lineWidth = 1.2 * this.scaleFactor;
    this.ctx.moveTo(x, y + apronH);
    this.ctx.lineTo(x + w, y + apronH);
    this.ctx.stroke();
  }
  ...
  drawCasementSash(x, y, w, h, sash, config) {
    ...
    // Remplissage du vantail (vitrage, panneau, soubassement). Sur un cintre
    // intégré, il est découpé par le contour cintré du vitrage.
    this.withArchGlassClip(() => {
      if (isDecoPanel) {
    ...
    // Traverse seule : même découpe, mais la partie basse reste vitrée.
    const validTrHeight = validSbHeight > 0 ? 0 : Math.min(config.traverse || 0, glassH - (40 * this.scaleFactor));
        ...
        if (validSbHeight > 0) {
        ...
        } else if (validTrHeight > 0) {
          const traverseThick = this.metrics.traverseThick;
          const topGlassH = glassH - validTrHeight - traverseThick;
          const bottomY = glassY + topGlassH + traverseThick;
          ...
          this.drawRect(glassX, glassY + topGlassH, glassW, traverseThick, this.colors.frame, this.colors.frameBorder, 1);
          // Partie basse vitrée (sans petits bois : ils sont comptés sur la partie haute).
          if (isOpaquePanel) {
            this.drawPVCPanel(glassX, bottomY, glassW, validTrHeight);
          } else {
            this.drawGlass(glassX, bottomY, glassW, validTrHeight);
          }
        } else {
          // Plein vitrage
          if (isOpaquePanel) {
    ...
    if (sash.symbols) {
      ...
      sash.symbols.forEach(symbol => {
        this.drawSymbol(x, y, w, h, symbol, fraction);
      });
    }
    ...
    // Poignée classique uniquement (plus de barre de tirage), y compris sur les
    // portes à panneau décoratif.
    if (sash.handle) {
      const fraction = config.handleHeightFraction ?? 0.5;
      this.drawHandle(x, y, w, h, sash.handle, false, fraction);
    }
  }
  ...
  drawSlidingSash(x, y, w, h, sash, index, totalSashes, config) {
    ...
    if (sash.symbols) {
      const fraction = config.handleHeightFraction ?? 0.5;
      sash.symbols.forEach(symbol => {
        this.drawSymbol(glassX, glassY, glassW, glassH, symbol, fraction);
      });
    }

    if (sash.handle) {
      this.drawHandle(adjustedX, y, adjustedW, h, sash.handle, true, config.handleHeightFraction ?? 0.5);
    }
  }
  ...
  drawSymbol(x, y, w, h, type, fraction = 0.5) {
    this.ctx.beginPath();
    ...
    const apexY = y + h * Math.min(1, Math.max(0, fraction));
    ...
    switch (type) {
      ...
      case 'triangle-left': // Poignée à GAUCHE (<)
        this.ctx.moveTo(x + w, y); this.ctx.lineTo(x, apexY); this.ctx.lineTo(x + w, y + h);
        break;
      case 'triangle-right': // Poignée à DROITE (>)
        this.ctx.moveTo(x, y); this.ctx.lineTo(x + w, apexY); this.ctx.lineTo(x, y + h);
        break;
  ...
  drawHandle(sashX, sashY, sashW, sashH, position, isSliding = false, fraction = 0.5) {
    ...
    // Centre de la poignée positionné selon la hauteur choisie, borné au vantail.
    const minY = sashY + offset;
    const maxY = sashY + sashH - h - offset;
    const handleY = Math.max(minY, Math.min(maxY, sashY + sashH * fraction - h / 2));
