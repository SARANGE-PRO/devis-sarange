          ...
          ${
            Number(amountDue) > 0
              ? `<div class="sg-block" style="background-color: #fff7ed; border: 1px solid #fdba74; border-left: 4px solid #f97316; padding: 20px; margin: 25px 0 0 0; border-radius: 8px;">
                   <p style="margin: 0; color: #9a3412; line-height: 1.6; font-size: 15px;">Le solde de <strong>${escapeHtml(formatCurrency(amountDue))}</strong> retenu lors de la réception devient dû à la signature du PV${paymentReference ? ` (référence ${escapeHtml(paymentReference)})` : ''}. Les coordonnées de règlement figurent sur le PV.</p>
                 </div>`
              : ''
          }
      ...
      await quoteRef.set(
        {
          completionWorkflow: {
            ...
            signedAt: now.toISOString(),
            hasReserves,
            // Pré-remplissage de la future modale de levée : montant retenu à
            // la réception, référence facture et e-mail utilisés sur ce bon.
...
export const createAndSendReservesLift = async ({
  ...
  // (retenue de 5 % pré-remplie, mais l'app ne sait pas ce qui a réellement
  // été payé entre-temps) : jamais recalculé d'autorité côté serveur.
