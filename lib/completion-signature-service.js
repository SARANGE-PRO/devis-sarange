...
export const submitGenericCompletion = async ({
  ...
  if (!clientName || !normalizeEnv(email) || !normalizeEnv(adresse) || !normalizeEnv(telephone)) {
    throw createHttpError('Coordonnées incomplètes.', 400);
  }
  if (!signatureDataUrl) {
    throw createHttpError('Signature manquante.', 400);
  }
  ...
  const { arrayBuffer, signatureAnchor, filename } = buildCompletionPdfDocument({
    quoteNumber: '',
    invoiceReference: '',
    clientData: { nom: clientName, adresseChantier: adresse, codePostalChantier: '', villeChantier: '' },
    ouvrages: [
      {
