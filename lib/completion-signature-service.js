...
const buildSignedConfirmationHtml = ({ clientName, hasReserves, completionNumber, allFiveStars = false, isLift = false }) =>
  getResponsiveEmailLayout({
    preheaderText: isLift
      ? 'Votre PV de levée des réserves signé est en pièce jointe. Votre dossier est clos !'
      : 'Votre bon de fin de chantier signé est en pièce jointe. Merci de votre confiance !',
    contentHtml: `
      ${getEmailHeaderHtml({ notificationLabel: isLift ? 'Levée des réserves' : 'Bon de fin de chantier' })}
      <tr>
        <td class="sg-content" style="padding: 40px 30px;">
          <h2 style="margin-top: 0; color: #0f172a; font-size: 20px;">Bonjour ${escapeHtml(clientName || 'Madame, Monsieur')},</h2>
          ${
          ...
          ${
            isLift
              ? `<p style="line-height: 1.6; color: #475569; font-size: 16px;">Vos ouvrages restent couverts par la garantie de bon fonctionnement (2 ans) et la garantie décennale (10 ans) depuis la date de réception initiale.</p>`
              : hasReserves
...
const buildInternalCompletionHtml = ({ clientName, completionNumber, hasReserves, quoteNumber, adresse, reserves = [], ratings = null }) => {
  ...
  const ratingLine = ratings
    ? `<p style="line-height: 1.6; color: #64748b; font-size: 14px;">Notes du client : pose ${ratings.pose}/5 · propreté ${ratings.proprete}/5 · relation ${ratings.relation}/5</p>`
    : '';
...
export const sendCompletionReminder = async ({ userId, sessionId, reminderLevel }) => {
  ...
  await transport.sendMail({
    ...
    replyTo: smtp.replyTo || undefined,
    subject: `⏰ Rappel : votre bon de fin de chantier attend votre signature (devis n°${(session.quoteNumber || '').replace(/^DV[-\s]*/i, '')})`,
...
// Photos de réserves : compressées côté client (JPEG ≤ 1280 px), plafonnées
// en nombre et en taille ici — le corps de requête serverless est limité à
// ~4,5 Mo, et cette route est atteignable sans authentification.
const MAX_RESERVE_PHOTOS = 4;
const MAX_PHOTO_DATAURL_LENGTH = 2_000_000;

const normalizeReservePhotos = (raw) =>
  (Array.isArray(raw) ? raw : [])
    .slice(0, MAX_RESERVE_PHOTOS)
    .map((dataUrl) => {
...
const sendSignedConfirmation = async ({
  ...
  // Photos de réserves jointes à l'e-mail INTERNE uniquement : elles servent
  // à préparer l'intervention de levée, pas à alourdir l'e-mail du client.
  ...
  if (toEmail) {
    try {
      await transport.sendMail({
        ...
        html: buildSignedConfirmationHtml({ clientName, hasReserves, completionNumber, allFiveStars, isLift }),
        attachments,
      });
    } catch (error) {
    ...
    try {
      await transport.sendMail({
        ...
        subject: isLift
          ...
          : `${hasReserves ? '⚠️ Bon signé AVEC réserves' : '✅ Bon de fin de chantier signé'} : ${
              clientName || 'Client'
            } (${completionNumber})${photos.length ? ` — ${photos.length} photo(s)` : ''}`,
        html: buildInternalCompletionHtml({ clientName, completionNumber, hasReserves, quoteNumber, adresse, reserves, ratings }),
...
export const signCompletionSession = async ({
  ...
  const normalizedPhotos = isLift ? [] : normalizeReservePhotos(reservePhotos);
  ...
  try {
    ...
    if (normalizedPhotos.length) {
      photoPaths = await uploadReservePhotos(sessionId, normalizedPhotos);
    }
  } catch (error) {
    await ref
...
export const submitGenericCompletion = async ({
  ...
  const normalizedPhotos = hasReserves ? normalizeReservePhotos(reservePhotos) : [];
  const photoPaths = normalizedPhotos.length
