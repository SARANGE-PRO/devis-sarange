...
const buildSignedConfirmationHtml = ({ clientName, hasReserves, completionNumber, allFiveStars = false }) =>
  getResponsiveEmailLayout({
  ...
  if (toEmail) {
    try {
      await transport.sendMail({
        ...
        html: buildSignedConfirmationHtml({ clientName, hasReserves, completionNumber, allFiveStars }),
        attachments,
      });
    } catch (error) {
...
export const signCompletionSession = async ({
  ...
  let filename;
  try {
    completionNumber = generateCompletionNumber(new Date());
    const built = buildCompletionPdfDocument({
