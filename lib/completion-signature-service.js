...
import { emitCompletionEvent } from './financial-events.mjs';
...
import { computeQuoteTotals } from './quote-totals.mjs';
...
const stripQuoteNumberPrefix = (value) => String(value || '').replace(/^DV[-\s]*/i, '');

const buildCompletionEmailSubject = (docType, quoteNumber) =>
  `📝 Votre ${getCompletionDocTypeLabel(docType)} SARANGE - Devis n°${stripQuoteNumberPrefix(quoteNumber)}`;

const buildCompletionReminderSubject = (docType, quoteNumber) =>
  `⏰ Rappel : votre ${getCompletionDocTypeLabel(docType)} attend votre signature (devis n°${stripQuoteNumberPrefix(quoteNumber)})`;

const buildLiftEmailSubject = (completionNumber) =>
  `📝 Levée des réserves de votre chantier - Bon n°${String(completionNumber || '').replace(/^BFC[-\s]*/i, '')}`;
...
const buildLiftEmailHtml = ({ clientName, signingUrl, completionNumber, amountDue = 0, paymentReference = '' }) =>
    ...
    contentHtml: `
      ...
      <tr>
        <td class="sg-content" style="padding: 40px 30px;">
          ...
          ${
            Number(amountDue) > 0
              ? `<div class="sg-block" style="background-color: #fff7ed; border: 1px solid #fdba74; border-left: 4px solid #f97316; padding: 20px; margin: 25px 0 0 0; border-radius: 8px;">
                   <p style="margin: 0; color: #9a3412; line-height: 1.6; font-size: 15px;">L'échéance finale de <strong>${escapeHtml(formatCurrency(amountDue))}</strong> prévue au devis devient exigible à la signature du PV${paymentReference ? ` (référence ${escapeHtml(paymentReference)})` : ''}. Les coordonnées de règlement figurent sur le PV.</p>
                 </div>`
              : ''
          }
...
const buildInternalRefusalHtml = ({ reason, isLift = false, clientName = '', completionNumber = '', quoteNumber = '' }) =>
  getResponsiveEmailLayout({
    preheaderText: isLift
      ? 'Un client signale que les réserves ne sont pas corrigées.'
      : 'Un client a refusé de signer un bon de fin de chantier.',
    contentHtml: `
      ${getEmailHeaderHtml({ accentColor: '#e11d48', subtitle: '', notificationLabel: 'Notification interne' })}
      <tr>
        <td class="sg-content" style="padding: 40px 30px;">
          <h2 style="margin-top: 0; color: #0f172a; font-size: 20px;">${
            isLift ? '⚠️ Levée des réserves refusée' : '⚠️ Bon de fin de chantier refusé'
          }</h2>
          ${
            isLift
              ? `<p style="line-height: 1.6; color: #475569; font-size: 16px;"><strong>${escapeHtml(clientName || 'Un client')}</strong> signale que les réserves du bon <strong>n°${escapeHtml(completionNumber)}</strong>${quoteNumber ? ` (devis n°${escapeHtml(quoteNumber)})` : ''} ne sont <strong>pas corrigées</strong>.</p>
                 <p style="line-height: 1.6; color: #475569; font-size: 16px;">Le dossier est repassé « réceptionné avec réserves » : planifiez une nouvelle intervention, puis renvoyez un PV de levée depuis la fiche devis.</p>`
...
/**
 * Résolution commune à l'envoi du bon et à son aperçu : devis, destinataire,
 * adresse du chantier, lignes de la variante qui fait foi, ouvrages et type
 * de document (pose / fourniture seule). Aucune écriture.
 */
const resolveCompletionSendContext = async ({
  userId,
  quoteId,
  deliveryType = '',
  overrideEmail = '',
  ...
  if (payload.variantsMode === true && Array.isArray(payload.variants) && payload.variants.length) {
    ...
    cartItems = Array.isArray(resolvedVariant.cartItems) ? resolvedVariant.cartItems : [];
    quoteSettings = resolvedVariant.quoteSettings || {};
  }
...
/**
 * Aperçu de l'e-mail du bon AVANT envoi : rien n'est créé ni envoyé.
 */
export const previewQuoteLinkedCompletion = async ({
  userId,
  quoteId,
  deliveryType = '',
  overrideEmail = '',
  ...
  const context = await resolveCompletionSendContext({
    userId,
    quoteId,
    deliveryType,
    overrideEmail,
  ...
  const quoteNumber = context.quoteData?.quoteNumber || '';
  const placeholderUrl = buildAbsoluteUrl(buildCompletionSignaturePageHref('nouveau-lien'));
  const html = buildCompletionEmailHtml({
    clientName: context.clientName,
    signingUrl: placeholderUrl,
    quoteNumber,
    docType: context.docType,
  });

  return {
    kind: 'completion-send',
    docType: context.docType,
    docLabel: getCompletionDocTypeLabel(context.docType),
    to: context.clientEmail,
    recipientName: context.clientName,
    subject: buildCompletionEmailSubject(context.docType, quoteNumber),
    text: htmlToPlainText(html).split(placeholderUrl).join('[lien personnel du bon, créé à l’envoi]'),
    attachments: [],
    link: '',
    linkIsNew: true,
    quoteNumber,
...
/**
 * Aperçu d'une relance du bon AVANT envoi : rien n'est envoyé.
 */
export const previewCompletionReminder = async ({ userId, sessionId, reminderLevel }) => {
  ensureServerConfig({ requireEmail: true });
  ...
  return {
    ...
    level: meta.level,
    docType: normalizeCompletionDocType(session.docType),
    docLabel: getCompletionDocTypeLabel(session.docType),
    to: session.clientEmail,
    recipientName: session.clientName || '',
    subject: buildCompletionReminderSubject(session.docType, session.quoteNumber),
...
export const createAndSendQuoteLinkedCompletion = async ({
  ...
  const {
    ...
    withPose,
    docType,
  } = await resolveCompletionSendContext({
    userId,
    quoteId,
    deliveryType,
    overrideEmail,
    requireEmail: deliveryMode === 'email',
  });

  const cgvSnapshotEntry = matchCgvSnapshotEntry(quoteData?.payload?.cgvSnapshot);
  // L'échéance finale en cas de réserves (art. 4.5 des CGV) ne concerne que
  // les marchés de TRAVAUX : jamais sur un bon d'enlèvement/livraison.
  const retentionEligible = withPose && cgvSnapshotEntryHasRetentionClause(cgvSnapshotEntry);

  // Montant qui fait foi : celui figé par le workflow de signature pour la
  // variante RETENUE (`selectedVariantTotalTTC`) ; à défaut (anciens devis
  // signés avant l'ajout de ce champ), on le recalcule depuis les lignes déjà
  // résolues ci-dessus. Jamais `quoteData.totalTTC` seul en mode variantes :
  // ce champ dénormalisé reflète la variante ACTIVE (celle éditée en dernier
  // dans /devis), qui peut différer de celle réellement signée par le client
  // — c'était le bug (bon de fin de chantier affichant le total d'une autre
  // variante que le devis signé).
  const totalDevisTTC =
    quoteData?.signatureWorkflow?.selectedVariantTotalTTC != null
      ? Number(quoteData.signatureWorkflow.selectedVariantTotalTTC)
      : resolvedVariant
        ? computeQuoteTotals(cartItems, resolvedVariant.tvaRate).totalTTC
        : Number(quoteData?.totalTTC) || 0;
  ...
  if (deliveryMode === 'email') {
    try {
      ...
      await transport.sendMail({
        ...
        subject: buildCompletionEmailSubject(docType, sessionDoc.quoteNumber),
  ...
  return { sessionId, signingUrl, deliveryMode, docType };
};
 ...
 */
/**
 * Aperçu d'un lien de bon ou de PV de levée pour les métadonnées de partage
 * (WhatsApp, SMS, e-mail). Lecture SANS effet de bord : contrairement à
 * getCompletionSession, la session n'est jamais marquée « consultée » (le
 * robot d'aperçu n'est pas le client).
 */
export const getCompletionSessionLinkPreview = async (sessionId) => {
  ensureServerConfig();

  const db = getFirebaseAdminDb();
  const snapshot = await readSessionDocumentRef(db, sessionId).get();
  if (!snapshot.exists) {
    throw createHttpError('Lien introuvable.', 404);
  }
  const session = snapshot.data();
  const status = normalizeCompletionStatus(session.status);
  ...
  return {
    mode: session.mode === 'reserves-lift' ? 'reserves-lift' : 'quote-linked',
    docType: normalizeCompletionDocType(session.docType),
...
export const getCompletionSession = async (sessionId) => {
  ...
  return {
    ...
    // et le montant qui deviendra dû à la signature (transparence : le client
    // doit le voir AVANT de signer, pas seulement sur le PDF).
    ...
    amountDue: data.mode === 'reserves-lift' ? Number(data.amountDue) || 0 : undefined,
...
export const signCompletionSession = async ({
  ...
  try {
    ...
    const built = await (isLift
      ? buildReservesLiftPdfDocument({
          ...
          amountDue: Number(session.amountDue) || 0,
      ...
      await quoteRef.set(
        {
          completionWorkflow: {
            ...
            signedAt: now.toISOString(),
            hasReserves,
            // Pré-remplissage de la future modale de levée : échéance finale
            // calculée à la réception, référence facture et e-mail de ce bon.
            retenueGarantie: balance.retenueGarantie || 0,
            invoiceReference: session.invoiceReference || '',
  ...
  // Événement financier vers sarange-finances (solde à la réception, ou
  // échéance finale de 5 % à la levée) — best-effort, jamais bloquant pour la
  // signature du bon ni pour les e-mails.
  await emitCompletionEvent(db, {
    ...session,
    id: sessionId,
    balance,
    completionNumber,
    // Numéro du bon d'origine (BFC-…) : sur un PV de levée, la variable
    // completionNumber ci-dessus est le numéro LR- fraîchement généré.
    originalCompletionNumber: isLift ? session.completionNumber || '' : '',
    reserves: normalizedReserves,
    signedAt: now.toISOString(),
  });
 ...
 */
/**
 * Résolution commune à l'envoi du PV de levée et à son aperçu : bon
 * d'origine réceptionné avec réserves, destinataire. Aucune écriture.
 */
const resolveLiftContext = async ({ userId, quoteId, overrideEmail = '', requireEmail = true }) => {
  const db = getFirebaseAdminDb();
  const quoteRef = readQuoteDocumentRef(db, userId, quoteId);
  const quoteSnapshot = await quoteRef.get();
  if (!quoteSnapshot.exists) {
  ...
  const clientEmail = normalizeEnv(overrideEmail) || original.clientEmail || '';
  if (requireEmail && !clientEmail) {
    throw createHttpError('Renseignez une adresse e-mail, ou créez simplement un lien à copier.', 400);
  }

  return { db, quoteRef, workflow, original, clientEmail };
};

/**
 * Aperçu de l'e-mail du PV de levée AVANT envoi : rien n'est créé ni envoyé.
 */
export const previewReservesLift = async ({
  userId,
  quoteId,
  amountDue = 0,
  paymentReference = '',
  ...
  const normalizedAmountDue = Math.max(0, Math.round((Number(amountDue) || 0) * 100) / 100);
  const normalizedPaymentReference = normalizeEnv(paymentReference).slice(0, 120);
  const { original, clientEmail } = await resolveLiftContext({
    userId,
    quoteId,
    overrideEmail,
    requireEmail: true,
  ...
  return {
    kind: 'lift-send',
    to: clientEmail,
    recipientName: original.clientName || '',
    subject: buildLiftEmailSubject(original.completionNumber),
    text: htmlToPlainText(html).split(placeholderUrl).join('[lien personnel du PV, créé à l’envoi]'),
    attachments: [],
    link: '',
    linkIsNew: true,
    amountDue: normalizedAmountDue,
...
export const createAndSendReservesLift = async ({
  userId,
  quoteId,
  amountDue = 0,
  paymentReference = '',
  ...
  // Le montant réclamé à la levée est SAISI PAR L'UTILISATEUR dans la modale
  // (échéance finale de 5 % pré-remplie, mais l'app ne sait pas ce qui a
  // réellement été payé entre-temps) : jamais recalculé d'autorité côté serveur.
  const normalizedAmountDue = Math.max(0, Math.round((Number(amountDue) || 0) * 100) / 100);
  const normalizedPaymentReference = normalizeEnv(paymentReference).slice(0, 120);

  const { db, quoteRef, workflow, original, clientEmail } = await resolveLiftContext({
    userId,
    quoteId,
    overrideEmail,
    requireEmail: deliveryMode === 'email',
  ...
  await readSessionDocumentRef(db, sessionId).set({
    ...
    completionNumber: original.completionNumber || '',
    // L'échéance finale n'a été réellement appliquée que si le bon d'origine
    // était éligible ET réceptionné avec réserves (toujours vrai ici).
...
export const refuseCompletionSession = async ({ sessionId, reason = '' }) => {
  ...
  const data = snapshot.data();
  const isLift = data.mode === 'reserves-lift';
  if (isLift && !['sent', 'viewed'].includes(data.status)) {
    throw createHttpError('Ce PV de levée a déjà été traité.', 409);
  }
  const now = new Date();
  ...
  // Refus d'un PV de levée = la réserve n'est PAS corrigée : le cycle repart
  // à zéro. Le devis revient à 'received_with_reserves' et le liftSessionId
  // est libéré pour qu'un NOUVEAU PV puisse être envoyé après une nouvelle
  // intervention — et ainsi de suite jusqu'à la levée effective.
  if (isLift && data.userId && data.quoteId) {
    await readQuoteDocumentRef(db, data.userId, data.quoteId)
      .set(
        {
          completionWorkflow: {
            status: 'received_with_reserves',
            liftSessionId: null,
            liftSentAt: null,
            lastLiftRefusal: { at: now.toISOString(), reason: trimmedReason },
          },
        },
        { merge: true }
      )
  ...
  if (smtp.host && smtp.fromEmail && smtp.internalEmail) {
    try {
      ...
      await transport.sendMail({
        ...
        to: smtp.internalEmail,
        subject: isLift
          ? `⚠️ Levée des réserves REFUSÉE : ${data.clientName || 'Client'} (${data.completionNumber || sessionId})`
          : `⚠️ Bon de fin de chantier refusé (${sessionId})`,
        html: buildInternalRefusalHtml({
          reason: trimmedReason,
          isLift,
          clientName: data.clientName || '',
          completionNumber: data.completionNumber || '',
          quoteNumber: data.quoteNumber || '',
        }),
      });
    } catch (error) {
...
export const submitGenericCompletion = async ({
  // Prestation choisie par le client à la première étape : 'reception'
  // (pose), 'livraison' ou 'enlevement'. Un seul lien pour les trois cas.
  docType = 'reception',
  nom,
  prenom,
  email,
  ...
  const safeCodePostal = cap(codePostal, 12);
  const safeVille = cap(ville, 120);
  // Borné d'abord, mis en forme ensuite (06 62 68 90 84) : le numéro remonte
  // tel quel dans la session Firestore et le récapitulatif.
  const safeTelephone = formatPhoneNumber(cap(telephone, 30));
  const safeQuoteReference = cap(quoteReference, 40);
  const safeDocType = normalizeCompletionDocType(docType);
  const isRemise = safeDocType !== 'reception';
  ...
  const { arrayBuffer, signatureAnchor, filename } = await buildCompletionPdfDocument({
    ...
    clientData: {
      nom: clientName,
      adresseChantier: safeAdresse,
      codePostalChantier: safeCodePostal,
      villeChantier: safeVille,
    },
    ouvrages: [
      {
        designation: isRemise ? 'Ensemble des produits remis' : 'Ensemble des travaux réalisés',
        repere: '',
        qte: 1,
      },
    ],
