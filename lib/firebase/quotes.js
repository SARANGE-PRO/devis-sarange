...
// Corbeille : collection séparée (même principe que commandes_corbeille dans
// sarange-suivi) plutôt qu'un simple flag sur le devis — un devis en corbeille
// disparaît ainsi naturellement de subscribeToUserQuotes sans logique de filtre.
const TRASH_RETENTION_DAYS = 30;

const getQuotesTrashCollection = (userId) => {
  const db = getFirebaseDb();
  if (!db) throw new Error('Firebase n’est pas configuré.');
  if (!userId) throw new Error('Utilisateur manquant.');
  return collection(db, 'users', userId, 'quotes_corbeille');
};

const getTrashDocument = (userId, quoteId) => doc(getQuotesTrashCollection(userId), quoteId);

const getTrashExpiry = (deletedAt) => {
...
//  • `cgvSnapshot` exclu → figement des CGV régénéré à chaque finalisation ;
//    ce n'est pas un changement de contenu du devis (sinon chaque génération de
//    PDF régénérerait le numéro et invaliderait la signature à tort).
  ...
  if (
    ...
    return Object.keys(value)
      ...
      .reduce((accumulator, key) => {
        if (key === 'customImage' || key === 'cgvSnapshot') return accumulator;
...
export async function saveQuoteDraft({
  ...
  variants,
  activeVariantId,
  // Snapshot de figement des CGV : fourni UNIQUEMENT à la finalisation
  // (génération PDF / envoi). Absent, l'éventuel snapshot déjà stocké est
  // conservé tel quel (merge Firestore) — jamais rafraîchi silencieusement.
  cgvSnapshot = undefined,
  status = null,
  ...
  const record = buildQuoteDraftRecord({
    ...
    cgvSnapshot: nextPayload.cgvSnapshot,
    status:
      status ||
...
export async function moveQuoteToTrash({ userId, quoteId }) {
  if (!quoteId) return;
  const quoteRef = getQuoteDocument(userId, quoteId);
  const snapshot = await getDoc(quoteRef);
  if (!snapshot.exists()) return;

  const deletedAt = new Date().toISOString();
  await setDoc(getTrashDocument(userId, quoteId), {
    ...snapshot.data(),
    deletedAt,
...
export async function restoreQuoteFromTrash({ userId, quoteId }) {
  if (!quoteId) return;
  const trashRef = getTrashDocument(userId, quoteId);
  const snapshot = await getDoc(trashRef);
  if (!snapshot.exists()) return;

  const quoteData = snapshot.data();
  delete quoteData.deletedAt;
  delete quoteData.expiresAt;
  await setDoc(getQuoteDocument(userId, quoteId), quoteData);
  await deleteDoc(trashRef);
}

export async function permanentlyDeleteTrashedQuote({ userId, quoteId }) {
  if (!quoteId) return;
  await deleteDoc(getTrashDocument(userId, quoteId));
}

const serializeTrashSnapshot = (snapshot) => {
  if (!snapshot?.exists()) return null;
...
// La purge (> 30 jours) se déclenche silencieusement à chaque abonnement, donc
// à chaque ouverture de « Mes devis » — aucune tâche planifiée nécessaire.
export function subscribeToTrashedQuotes({ userId, onNext, onError }) {
  const trashQuery = query(getQuotesTrashCollection(userId), orderBy('deletedAt', 'desc'));

  return onSnapshot(
    trashQuery,
    (snapshot) => {
      const items = snapshot.docs.map((entry) => serializeTrashSnapshot(entry)).filter(Boolean);
      const now = Date.now();
      const valid = [];
      ...
      items.forEach((item) => {
        if (new Date(item.expiresAt).getTime() > now) {
          ...
          void deleteDoc(getTrashDocument(userId, item.id)).catch(() => undefined);
        }
      });
