...
const serializeClientSnapshot = (snapshot) => {
  ...
  const payload = sanitizeClientData(data?.payload);

  return {
    id: snapshot.id,
    ...data,
    // Le numéro de la fiche suit la mise en forme du payload normalisé, même
    // pour les fiches enregistrées avant (06 62 68 90 84).
    telephone: payload.telephone || formatPhoneNumber(data?.telephone) || null,
    payload,
  };
};
