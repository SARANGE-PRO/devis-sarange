...
import { resolveEffectiveTvaRate } from '@/lib/vat-window-eligibility.mjs';
...
/**
 * Taux de TVA EFFECTIF d'une ligne : c'est ce taux — jamais le taux brut
 * `item.tvaRate` — qui doit être utilisé pour tout calcul (totaux, PDF,
 * export Sage). Un taux à 5,5 % demandé sur une menuiserie dont le Uw/Sw
 * (ou l'Ud, pour une porte) ne respecte pas les seuils légaux est
 * AUTOMATIQUEMENT ramené à 10 % — voir lib/vat-window-eligibility.mjs pour le
 * détail des seuils et la justification. Aucun autre taux n'est retouché.
 */
export const getItemTvaRate = (item, defaultTvaRate = DEFAULT_TVA_RATE) => {
  const nominalRate = normalizeTvaRate(item?.tvaRate, defaultTvaRate);
  return resolveEffectiveTvaRate(item, nominalRate).rate;
};

/**
 * Détail de la résolution du taux (pour affichage d'un avertissement quand la
 * ligne a été ramenée de 5,5 % à 10 %). `getItemTvaRate` reste la fonction à
 * utiliser pour tout calcul ; celle-ci n'ajoute que l'explication.
 */
export const getItemTvaCorrection = (item, defaultTvaRate = DEFAULT_TVA_RATE) => {
  const nominalRate = normalizeTvaRate(item?.tvaRate, defaultTvaRate);
  return resolveEffectiveTvaRate(item, nominalRate);
};
