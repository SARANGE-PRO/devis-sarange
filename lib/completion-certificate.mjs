...
/**
 * Libellé du document de réception d'un devis. Une fois le bon envoyé, son
 * type (`docType`) fait foi ; avant, seule la présence de pose est connue :
 * avec pose → bon de fin de chantier ; fourniture seule → « bon de
 * livraison » par défaut (le choix enlèvement / livraison n'est fait qu'à
 * l'envoi, manuellement). Jamais « fin de chantier » sans pose.
 */
export const getCompletionDocumentLabel = ({ docType = '', withPose = true } = {}) => {
  if (COMPLETION_DOC_TYPES.includes(docType)) return getCompletionDocTypeLabel(docType);
  return withPose ? 'bon de fin de chantier' : 'bon de livraison';
};

export const capitalizeLabel = (label) =>
  typeof label === 'string' && label ? label.charAt(0).toUpperCase() + label.slice(1) : '';

/* ─── Adresse et date des documents ──────────────────────────────────────── */
...
/**
 * Adresse du chantier sur une ligne, sans doublon : le code postal et la
 * ville ne sont ajoutés que s'ils ne figurent pas déjà dans l'adresse (le
 * bon général reçoit l'adresse complète de l'autocomplétion, ville comprise).
 */
export const composeCompletionAddress = (adresse, codePostal, ville) => {
  const street = String(adresse || '').trim();
  const postalCode = String(codePostal || '').trim();
  const city = String(ville || '').trim();
  const folded = foldText(street);

  const parts = [street];
  if (postalCode && !folded.includes(foldText(postalCode))) parts.push(postalCode);
  if (city && !folded.includes(foldText(city))) parts.push(city);

  return parts.filter(Boolean).join(', ');
};

/**
 * Date ET heure de signature, à l'heure de Paris quel que soit le serveur
 * (« 22/09/2026 à 14:35 »). Chaîne vide si la date est invalide.
 */
export const formatCompletionDateTime = (date) => {
  const value = date instanceof Date ? date : new Date(date);
  if (Number.isNaN(value.getTime())) return '';

  const day = new Intl.DateTimeFormat('fr-FR', { timeZone: 'Europe/Paris' }).format(value);
