...
export const DEFAULT_PRICING = Object.freeze({
  ...
  // Coefficient de marge appliqué au prix d'achat net de chaque pièce
  // détachée (cf. lib/spare-parts.js). Par défaut x2.
  sparePartsMarkupCoefficient: 2,
  // Surcharges ponctuelles du prix d'achat d'une référence pièce détachée,
  // sans toucher au catalogue — même principe que glazingPrices.
  sparePartsPrices: Object.freeze({}),
});
...
export const sanitizePricing = (value) => {
  ...
  const sparePartsPrices =
    raw.sparePartsPrices && typeof raw.sparePartsPrices === 'object' ? raw.sparePartsPrices : {};
  ...
  return {
    ...
    sparePartsMarkupCoefficient: Math.max(
      0,
      toNumber(raw.sparePartsMarkupCoefficient, DEFAULT_PRICING.sparePartsMarkupCoefficient)
    ),
    sparePartsPrices: Object.entries(sparePartsPrices).reduce((accumulator, [partId, price]) => {
      const normalized = toNumber(price, null);
      if (normalized === null) return accumulator;
      accumulator[partId] = Math.max(0, normalized);
      return accumulator;
    }, {}),
  };
};
...
export function getCataloguePricing() {
  ...
  return {
    ...
    sparePartsPrices: { ...pricing.sparePartsPrices },
  };
}
...
export function removeGlazingPrice(glazingId) {
  if (!glazingId) return getCataloguePricing();
  const current = loadCataloguePricing();
  const nextGlazingPrices = { ...current.glazingPrices };
  delete nextGlazingPrices[glazingId];

  return persistCataloguePricing({
    ...current,
    glazingPrices: nextGlazingPrices,
  });
}

export function setSparePartPrice(partId, value) {
  if (!partId) return getCataloguePricing();
  const current = loadCataloguePricing();
  return persistCataloguePricing({
    ...current,
    sparePartsPrices: {
      ...current.sparePartsPrices,
      [partId]: value,
    },
  });
}

export function removeSparePartPrice(partId) {
  if (!partId) return getCataloguePricing();
  const current = loadCataloguePricing();
  const nextSparePartsPrices = { ...current.sparePartsPrices };
  delete nextSparePartsPrices[partId];

  return persistCataloguePricing({
    ...current,
    sparePartsPrices: nextSparePartsPrices,
  });
}
...
export function hydrateCataloguePricing(pricing) {
  return persistCataloguePricing(pricing);
}
