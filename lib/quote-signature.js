...
/**
 * Recherche par numéro de devis : « DV-262341234 », « dv 262341234 »,
 * « 262341234 » ou seulement une partie du numéro (au moins trois chiffres)
 * retrouvent le même devis. Le préfixe « DV » et les séparateurs sont
 * facultatifs des deux côtés.
 */
export const quoteNumberMatchesSearch = (quoteNumber, term) => {
  const needle = (typeof term === 'string' ? term : '').replace(/[\s._/-]/g, '').toLowerCase();
  const match = /^(?:dv)?(\d{3,})$/.exec(needle);
  if (!match) return false;

  const digits = getQuoteNumberDisplay(quoteNumber).replace(/\D/g, '');
  return digits.length > 0 && digits.includes(match[1]);
};

// Numéro de devis : DV-{AA}{JJJ}{HHmm} (année sur 2 chiffres, jour de l'année sur 3,
// heure+minute). Source de vérité UNIQUE (client + PDF). Le numéro est figé à la
// première sauvegarde d'un devis et régénéré uniquement quand son contenu change.
...
export const buildSignatureAttachmentHref = (sessionId, index) =>
  `/api/quote-signatures/${encodeURIComponent(String(sessionId || ''))}/attachment?index=${encodeURIComponent(String(index))}`;
