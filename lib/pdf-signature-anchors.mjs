 ...
 * @param {import('pdf-lib').PDFPage | null} fallbackPage  peut être nul (voir `drawAtAnchor`)
 */
...
/**
 * Point d'entrée UNIQUE pour dessiner un élément rattaché à un repère.
 *
 * >>> RÉCIDIVE CONNUE : la croix de certification TVA réduite a déjà été
 * dessinée sur la page de signature au lieu de la sienne (bug corrigé une
 * première fois, puis réapparu). Cause à chaque fois la même : quelqu'un
 * dessine directement sur une page capturée plus haut (`page`,
 * `signaturePage`…) au lieu de résoudre la page PROPRE au repère. <<<
 *
 * `drawAtAnchor` rend cette erreur impossible : il n'y a pas d'accès à une
 * page tant que celle du repère n'a pas été résolue. Tout nouveau repère
 * ajouté à `signaturePdfAnchors` (case à cocher, tampon, mention…) DOIT
 * passer par cette fonction — ne jamais dessiner un élément de repère via la
 * variable `page`/`signaturePage` capturée en haut de `applySignatureToPdf`,
 * qui ne vaut QUE pour le bloc « Bon pour accord ».
 *
 * Si la page ne peut pas être résolue et qu'aucune page de repli n'est
 * fournie (`fallbackPage` nul), l'élément est silencieusement omis plutôt que
 * de risquer un dessin au mauvais endroit ou un crash.
 *
 * @param {import('pdf-lib').PDFDocument} pdfDocument
 * @param {{pageNumber?: number}} anchor
 * @param {import('pdf-lib').PDFPage | null} fallbackPage
 * @param {(page: import('pdf-lib').PDFPage, pageHeight: number) => void} draw
 */
export const drawAtAnchor = (pdfDocument, anchor, fallbackPage, draw) => {
  const targetPage = resolveAnchorPage(pdfDocument, anchor?.pageNumber, fallbackPage);
  if (!targetPage) {
    return;
  }
  draw(targetPage, targetPage.getHeight());
};
