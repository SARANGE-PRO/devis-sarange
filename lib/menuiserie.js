...
export const buildMenuiserieConfig = ({ sheetName, width, height, options = {} }) => {
  ...
  return {
    ...
    // Traverse seule : la partie basse reste vitrée. Priorité au soubassement.
    traverse: options.hasTraverse && !options.hasSousBassement ? options.traverseHeight : 0,
...
export const buildCompositeModuleConfig = ({ module, height, options = {} }) => {
  ...
  return buildMenuiserieConfig({
    ...
    options: {
      ...
      hasTraverse: module?.options?.hasTraverse ?? options.hasTraverse,
      traverseHeight: module?.options?.traverseHeight ?? options.traverseHeight,
