...
const DEFAULT_SETTINGS = Object.freeze({
  ...
  // Commission commerciale / apporteur d'affaires : prélevée sur le total HT après
  // remise et redistribuée DANS les prix des menuiseries (au prorata), sans toucher
  // à la pose. Invisible pour le client (aucune ligne « commission »). Deux modes de
  // saisie, `commissionMode` choisit lequel s'applique — l'autre champ reste en
  // mémoire mais n'est pas utilisé, pour ne rien perdre en changeant d'avis.
...
export const normalizeQuoteSettings = (input = {}) => {
  ...
  return {
    ...
    commissionMode: nextSettings.commissionMode === 'amount' ? 'amount' : 'percent',
    commissionPercent: normalizePercent(nextSettings.commissionPercent),
    commissionAmount: Math.max(0, roundCurrency(nextSettings.commissionAmount)),
  };
};
