...
// « À la commande » → « à la commande » pour insertion en milieu de phrase.
const lowercaseDue = (dueLabel) => {
  const text = normalizeString(dueLabel);
...
export const buildPaymentTermsSentence = (input = {}) => {
  ...
  if (settings.paymentMode === 'schedule') {
    const dues = [1, 2, 3].map(
      (step) => lowercaseDue(getScheduleTrigger(settings, step).dueLabel) || 'à préciser'
    );
    return [
  ...
  return [
    `Règlement d'un acompte de ${depositPercent}% à la commande,`,
    "ou par chèque ou virement lors de la prise de côtes si un métré SARANGE est prévu,",
...
export const buildPaymentTermsForPdf = (input = {}) => {
  ...
  return [
    ...
    "Le règlement peut s'effectuer par virement bancaire, carte bancaire ou chèque. Un règlement par virement n'est considéré comme effectué qu'après crédit effectif du compte bancaire de SARANGE.",
