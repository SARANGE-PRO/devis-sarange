...
/**
 * Lignes et réglages qui FONT FOI pour un devis : la variante retenue à la
 * signature, sinon la variante active, sinon la première ; le devis lui-même
 * en mono-variante. Même règle que le bon de fin de chantier et la TVA.
 */
export const resolveQuoteVariantSource = (quote) => {
  ...
  const wantedId = quote?.signatureWorkflow?.selectedVariantId || payload.activeVariantId || '';
  const variant = payload.variants.find((entry) => entry?.id === wantedId) || payload.variants[0] || {};
...
/** Type de contrat effectif d'un devis (variante qui fait foi + correction manuelle). */
export const resolveQuoteContractType = (quote) => {
  const source = resolveQuoteVariantSource(quote);
  return resolveContractType(source.cartItems, source.quoteSettings?.contractTypeOverride);
};

export const quoteIncludesPose = (quote) =>
  resolveQuoteContractType(quote) === CONTRACT_TYPES.AVEC_POSE;
