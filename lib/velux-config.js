...
import { VELUX_GLAZING_THERMAL } from './vat-window-eligibility.mjs';
...
export const VELUX_OPENINGS = [
  {
    ...
    // Fichier « -v2 » : version détourée (nom changé pour purger les caches
    // CDN/navigateur des anciennes images à fond blanc).
  ...
  },
  {
    ...
    imageSrc: '/fenetre-de-toit-velux-projection-v2.webp',
  },
];
 ...
 */
/**
 * Performance thermique de la gamme de vitrage choisie.
 *
 * Source unique : VELUX_GLAZING_THERMAL (lib/vat-window-eligibility.mjs), déjà
 * sourcée sur le tarif professionnel Velux France. On ne recopie pas ces
 * valeurs ici : elles justifient un taux de TVA auprès de l'administration,
 * deux tables qui divergent seraient un risque fiscal.
 *
 * Velux publie une valeur DÉCLARATIVE PAR GAMME de vitrage, et non par code
 * dimensionnel : la performance ne varie donc pas avec la taille choisie.
 */
export const getVeluxThermal = (rangeId) => VELUX_GLAZING_THERMAL[rangeId] || null;

/**
 * Uw/Sw formatés pour le descriptif, dans la même forme que les menuiseries
 * classiques (cf. lib/designation-generator.js) : « Uw = 1.3 W/m²K – Sw = 0.23 ».
 */
export const formatVeluxThermal = (rangeId) => {
  const thermal = getVeluxThermal(rangeId);
  if (!thermal) return '';
  return `Uw = ${thermal.uw} W/m²K – Sw = ${thermal.sw}`;
};
...
export const buildVeluxDesignation = (selection = {}) => {
  ...
  const withFlashing = `${withAccessory} + Raccord ${flashing.code} (${flashing.label})`;

  // Uw/Sw dans le descriptif : c'est la mention qui justifie le taux de 5,5 %
  // sur la facture et le devis, au même titre que pour les autres menuiseries.
  const thermal = formatVeluxThermal(range.id);
  return thermal ? `${withFlashing} – ${thermal}` : withFlashing;
};
 ...
 * @property {{opening: string, finish: string, finishCommercialName: string, size: string, range: string, flashing: string, accessory: string, pose?: string}} labels
 *   Libellés lisibles de chaque choix (pour affichage devis / récapitulatif)
 * @property {{included: boolean, priceHt: number}} [pose]
 *   Pose sur site (ajoutée par le configurateur UI : prix HT libre saisi par
 *   l'utilisateur, reporté en sous-ligne « Pose » du devis — hors référence)
 */
...
export const createVeluxConfiguration = (selection = {}) => {
  ...
  return {
    ...
    prefix,
    designation,
    // Mêmes champs que les menuiseries classiques : la ligne de devis porte
    // sa performance thermique, et le contrôle d'éligibilité au taux de 5,5 %
    // comme le PDF y puisent sans retraiter le libellé.
    thermalUw: getVeluxThermal(range.id)?.uw ?? null,
    thermalSw: getVeluxThermal(range.id)?.sw ?? null,
