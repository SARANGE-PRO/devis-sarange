...
import { applyCintrageToPricing, normalizeCintrage } from '@/lib/cintrage.mjs';
...
// Feuille de référence pour les coulissants sans grille fournisseur dédiée
// (3V/4V PVC et alu, galandages) : grille dimensionnelle du coulissant 2V 2R,
// prix à 0 € en attendant les tarifs réels.
...
export const CATEGORIES = [
  ...
  },
  {
    ...
    products: [
      ...
      // Coulissants 3/4 vantaux : pas de grille fournisseur PVC. Grille
      // dimensionnelle empruntée au 2V 2R, prix à 0 € en attendant les tarifs
      // réels (le jumeau ALU généré automatiquement reste lui aussi à 0 €).
      {
        id: 'coulissant-3v',
        sheet: 'Coulissant 3 vantaux',
  ...
  },
  {
    ...
    products: [
      {
        id: 'piece-moteurs-emetteurs',
        sheet: 'Piece Detachee',
        label: 'Moteurs & émetteurs',
        shortLabel: 'Moteurs & émetteurs',
        pricingMode: 'spare-part',
        sparePartCategoryId: 'moteurs-emetteurs',
        requiresDimensions: false,
        supportsPose: false,
        supportsThermalData: false,
      },
      {
        id: 'piece-tabliers-lames',
        sheet: 'Piece Detachee',
        label: 'Tablier de volet roulant (au m²)',
        shortLabel: 'Tablier (m²)',
        pricingMode: 'spare-part-tablier',
      ...
      },
      {
        id: 'piece-coffres',
        sheet: 'Piece Detachee',
        label: 'Coffres aluminium',
        shortLabel: 'Coffres',
        pricingMode: 'spare-part',
        sparePartCategoryId: 'coffres',
        requiresDimensions: false,
        supportsPose: false,
        supportsThermalData: false,
      },
      {
        id: 'piece-flasques',
        sheet: 'Piece Detachee',
        label: 'Flasques & contreplaques',
        shortLabel: 'Flasques',
        pricingMode: 'spare-part',
        sparePartCategoryId: 'flasques',
        requiresDimensions: false,
        supportsPose: false,
        supportsThermalData: false,
      },
      {
        id: 'piece-encadrement',
        sheet: 'Piece Detachee',
        label: 'Encadrement',
        shortLabel: 'Encadrement',
        pricingMode: 'spare-part',
        sparePartCategoryId: 'encadrement',
        requiresDimensions: false,
        supportsPose: false,
        supportsThermalData: false,
      },
      {
        id: 'piece-mecanique',
        sheet: 'Piece Detachee',
        label: 'Mécanique & composants',
        shortLabel: 'Mécanique',
        pricingMode: 'spare-part',
        sparePartCategoryId: 'mecanique',
        requiresDimensions: false,
        supportsPose: false,
        supportsThermalData: false,
      },
...
// Familles de la catégorie « Pièces détachées » (cf. lib/spare-parts.js pour
// le catalogue des références). Prix fixe déjà calculé (achat × coefficient)
// au moment de l'ajout au panier : même traitement que le métrage/forfait de
// déplacement dans calculateItemPrice (aucun coefficient/remise/commission/
// pose supplémentaire).
export const SPARE_PART_PRODUCT_IDS = new Set([
  'piece-moteurs-emetteurs',
  'piece-tabliers-lames',
  'piece-coffres',
  'piece-flasques',
  'piece-encadrement',
  'piece-mecanique',
...
const CANONICAL_SHEET_BY_PRODUCT_ID = {
  ...
  'coulissant-3v': 'Coulissant 3 vantaux',
  'coulissant-4v': 'Coulissant 4 vantaux',
  ...
  'piece-moteurs-emetteurs': 'Piece Detachee',
  'piece-tabliers-lames': 'Piece Detachee',
  'piece-coffres': 'Piece Detachee',
  'piece-flasques': 'Piece Detachee',
  'piece-encadrement': 'Piece Detachee',
  'piece-mecanique': 'Piece Detachee',
};

// ─── Formes spéciales : menuiseries cintrées ──────────────────────────────
// Fenêtres, portes-fenêtres et portes d'entrée avec cintrage (fixe cintré en
// imposte fabriqué en interne, ou cintre intégré à l'ouvrant sous-traité).
// Chaque produit cintré est ADOSSÉ à un produit standard (`basedOn`) : même
// feuille tarifaire (la partie basse se chiffre sur la grille habituelle) et
// même coefficient catalogue (`coefficientSource`). Les jumeaux aluminium sont
// générés comme pour les autres catégories. Jamais proposés dans le composé.
// Le prix propre au cintrage vient de lib/cintrage.mjs (cf. calculateItemPrice).
export const CINTRE_CATEGORY_ID = 'formes-speciales';

const CINTRE_PRODUCT_DEFINITIONS = [
  { basedOn: 'fenetre-1v', label: 'Fenêtre 1 vantail cintrée', shortLabel: '1V cintrée' },
  { basedOn: 'fenetre-2v', label: 'Fenêtre 2 vantaux cintrée', shortLabel: '2V cintrée' },
  { basedOn: 'fenetre-3v', label: 'Fenêtre 3 vantaux cintrée', shortLabel: '3V cintrée' },
  { basedOn: 'fenetre-4v', label: 'Fenêtre 4 vantaux cintrée', shortLabel: '4V cintrée' },
  { basedOn: 'fenetre-2v1f', label: 'Fenêtre 2V + 1 fixe cintrée', shortLabel: '2V+1F cintrée' },
  { basedOn: 'fenetre-2v2f', label: 'Fenêtre 2V + 2 fixes cintrée', shortLabel: '2V+2F cintrée' },
  { basedOn: 'fenetre-fixe', label: 'Fenêtre fixe cintrée', shortLabel: 'Fixe cintrée' },
  { basedOn: 'pf-1v', label: 'Porte-fenêtre 1 vantail cintrée', shortLabel: 'PF 1V cintrée' },
  { basedOn: 'pf-2v', label: 'Porte-fenêtre 2 vantaux cintrée', shortLabel: 'PF 2V cintrée' },
  { basedOn: 'pf-3v', label: 'Porte-fenêtre 3 vantaux cintrée', shortLabel: 'PF 3V cintrée' },
  { basedOn: 'pf-2v1f', label: 'Porte-fenêtre 2V + 1 fixe cintrée', shortLabel: 'PF 2V+1F cintrée' },
  { basedOn: 'pf-2v2f', label: 'Porte-fenêtre 2V + 2 fixes cintrée', shortLabel: 'PF 2V+2F cintrée' },
  // Le suffixe « – rénovation / – neuf » est retiré à l'affichage client
  // (getDisplayProductLabel), comme pour les portes standard.
  { basedOn: 'porte-reno', label: "Porte d'entrée cintrée – rénovation", shortLabel: 'Porte réno cintrée' },
  { basedOn: 'porte-neuf', label: "Porte d'entrée cintrée – neuf", shortLabel: 'Porte neuf cintrée' },
];

const findCatalogProductDefinition = (productId) => {
  for (const category of CATEGORIES) {
    const match = category.products.find((entry) => entry.id === productId);
    if (match) return match;
  }
  return null;
};

const cintreProducts = CINTRE_PRODUCT_DEFINITIONS.flatMap(({ basedOn, ...definition }) => {
  const base = findCatalogProductDefinition(basedOn);
  if (!base) return [];
  return [
    {
      id: `${basedOn}-cintre`,
      ...
      basedOn,
      shape: 'cintre',
      coefficientSource: base.coefficientSource || basedOn,
      compositeEligible: false,
      ...definition,
    },
  ];
...
CATEGORIES.splice(CATEGORIES.findIndex((category) => category.id === 'portes') + 1, 0, {
  id: CINTRE_CATEGORY_ID,
  label: 'Formes spéciales',
  icon: 'Sunrise',
  products: cintreProducts,
});

cintreProducts.forEach((product) => {
  CANONICAL_SHEET_BY_PRODUCT_ID[product.id] = CANONICAL_SHEET_BY_PRODUCT_ID[product.basedOn];
});
...
// Produits galandage proposés uniquement en aluminium (gamme Schüco
...
CATEGORIES.forEach((category) => {
  category.products.forEach((product) => {
    ...
    // Un produit adossé (menuiserie cintrée) partage la feuille de son produit
    // de base : la résolution par nom de feuille reste celle du produit standard.
    if (product.basedOn) return;
...
CATEGORIES.forEach((category) => {
  category.products.forEach((product) => {
    const sheet = CANONICAL_SHEET_BY_PRODUCT_ID[product.id] || product.sheet;
    if (PRICING[sheet]) return;
    // Jumeau alu : grille du produit PVC d'origine (y compris grilles dérivées
    // comme la PF 3V). Produit sans grille fournisseur (coulissants 3V/4V PVC,
    // galandages alu-only) : grille empruntée à une feuille de référence
    // (pricingSource). Les PVC d'une catégorie précèdent leurs jumeaux alu,
    // donc la feuille PVC générée existe quand le jumeau vient l'emprunter.
...
/**
 * Couleur à utiliser sur les schémas (toujours la face intérieure de la
 * menuiserie) à partir de l'option de coloration choisie. Logique partagée
 * entre le formulaire (ProductSelector) et le rendu PDF des châssis composés,
 * pour éviter qu'un châssis composé reste blanc sur le schéma faute de
 * recalcul de cette couleur par module.
 */
export function resolveSvgColor({ isVolet = false, colorOptionId, colorState } = {}) {
  const state = cloneColorState(colorState);

  if (isVolet && colorOptionId === 'coloration-2f') {
    if (state.color2fType === 'standard_7016') return '#4A4A4A';
    if (state.color2fType === 'standard_chene') return '#8B5A2B';
    return state.customColor2fHex || '#4A4A4A';
  }

  if (!isVolet && colorOptionId === 'bicoloration') {
    if (state.bicoType === 'standard_7016' || state.bicoType === 'standard_chene') {
      return '#FFFFFF';
    }
    const inside = state.customColorIntText || 'Interieur a definir';
    return inside.toLowerCase().includes('blanc') || !inside.trim()
      ? '#FFFFFF'
      : state.customColorIntHex || '#FFFFFF';
  }
...
const normalizeModuleOptions = (productId, options = {}, legacySource = {}) => ({
  ...
  // Traverse seule (vitrée en dessous) : même hauteur visible que le soubassement.
  hasTraverse: Boolean(options.hasTraverse ?? legacySource.hasTraverse),
  traverseHeight: parsePositiveNumber(options.traverseHeight ?? legacySource.traverseHeight, 400),
...
/**
 * Catégorie de référence pour les règles métier « par famille » (déchets…) :
 * une menuiserie cintrée suit la famille de son produit de base (fenêtre,
 * porte-fenêtre, porte d'entrée).
 */
export function getProductFamilyCategory(productId) {
  const product = getProductById(productId);
  if (!product) return null;
  return product.basedOn ? getProductCategory(product.basedOn) : product.categoryId;
}
...
const buildModulePricingData = (module) => {
  ...
  const glassAreas =
    isGlazed && frameSystem
      ? calculateGlassAreas(widthMm, heightMm, frameSystem, product.sheet)
      : null;
  const fillingPricing =
    isGlazed && glassAreas
      ? calculateGlazingAndPanelExtras({
          ...
          hasTraverse: options.hasTraverse,
          colorOptionId: options.colorOptionId,
          isAlu: product.material === 'alu',
        })
        : {
          glazingExtra: 0,
  ...
  // Châssis hors grille : tarif de repli à la plus grande cote de la grille.
  const fallbackPriceData =
    priceData ?? getFallbackPriceForMm(product.sheet, heightMm, widthMm);

  const adjustBasePrice = (rawPrice) => {
    if (rawPrice === null || rawPrice === undefined) return null;
    const withCoefficient = applyProductCoefficient(
      rawPrice,
      resolveCoefficientProductId(product.id)
    );
    return colorOption?.surcharge > 0
      ? withCoefficient * (1 + colorOption.surcharge)
      : withCoefficient;
  };

  const modulePrice = adjustBasePrice(priceData?.price ?? null);
  const fallbackModulePrice = adjustBasePrice(fallbackPriceData?.price ?? null);
  ...
  return {
    ...
    // Prix de repli (cote hors grille ramenée au maximum de la grille) : utilisé
    // uniquement par les composés ajoutés hors grille sur double validation.
    fallbackPriceData,
    fallbackUnitPrice:
      fallbackModulePrice === null
        ? null
        : roundCurrency(fallbackModulePrice + optionsPrice),
    isGlazed,
  };
};
...
const calculateSimpleItemThermalMetrics = (item) => {
  ...
  const normalizedSheetName = normalizeSheetName(item.sheetName);
  const frameSystem = getFrameSystemForProduct(normalizedSheetName);
...
export function getCompositePricing(composition = [], legacyModules = null) {
  ...
  return {
    ...
    // Total « forcé » : les châssis hors grille sont tarifés au repli (cote
    // ramenée au maximum de la grille). Utilisé uniquement par les lignes
    // ajoutées hors grille sur double validation, où la marge nette souhaitée
    // vient compenser le hors-format.
    forcedTotalPrice:
      modulePricing.length === 0
        ? null
        : roundCurrency(
            modulePricing.reduce(
              (total, module) => total + (module.unitPrice ?? module.fallbackUnitPrice ?? 0),
              0
            )
          ),
    hasInvalidModule,
...
export function getCompositeFramePricing(frame) {
  ...
  return {
    ...
    // Total « forcé » pour l'ajout hors grille sur double validation : les
    // châssis hors grille sont tarifés au repli (cote ramenée au maximum de la
    // grille). Reste `null` si la géométrie est irrésoluble ou si le composé
    // est vide : ces deux cas ne sont pas des problèmes de grille et restent
    // bloquants.
    forcedTotalPrice:
      !solved.ok || isEmpty
        ? null
...
/**
 * Résolution d'une case de grille tarifaire : on facture la première cote >= à
 * la cote demandée (arrondi supérieur). `null` si la mesure sort de la grille.
 */
const resolveGridEntry = (sheetName, heightMm, widthMm) => {
  const data = PRICING[normalizeSheetName(sheetName)];
  if (!data?.heights?.length || !data.prices?.length) return null;
...
export function getPriceForMm(sheetName, heightMm, widthMm) {
  return resolveGridEntry(sheetName, heightMm, widthMm);
}

// Nombre de pas de fin de grille retenus pour estimer le €/cm marginal.
const OUT_OF_GRID_STEP_SAMPLE = 5;
// Garde-fou anti-faute de saisie : une case ne peut pas servir de référence si
// elle vaut plus que ce multiple du prix de la plus grande cote de sa ligne /
// colonne (certaines feuilles contiennent des valeurs manifestement erronées).
const OUT_OF_GRID_PRICE_GUARD = 1.5;
...
/**
 * €/cm marginal en fin de grille, sur une suite de cases d'un même axe.
 * On ne garde que les pas POSITIFS (plusieurs feuilles contiennent des
 * inversions de prix) et on prend la médiane des derniers pas : une valeur
 * aberrante isolée ne peut donc pas emballer l'extrapolation.
 */
const marginalRatePerCm = (cells) => {
  const sorted = [...cells].sort((left, right) => left.cote - right.cote);
  const steps = [];
  for (let index = 1; index < sorted.length; index += 1) {
    const deltaCote = sorted[index].cote - sorted[index - 1].cote;
    const deltaPrice = sorted[index].prix - sorted[index - 1].prix;
    if (deltaCote > 0 && deltaPrice > 0) steps.push(deltaPrice / deltaCote);
  }
  return median(steps.slice(-OUT_OF_GRID_STEP_SAMPLE));
};

// Case la plus chère d'une ligne / colonne, en écartant les valeurs aberrantes
// (cf. OUT_OF_GRID_PRICE_GUARD). `extremeCell` sert de référence de plausibilité.
const mostExpensiveCell = (cells, extremeCell) => {
  const ceiling = extremeCell ? extremeCell.prix * OUT_OF_GRID_PRICE_GUARD : Infinity;
  return cells.reduce((best, cell) => {
    if (cell.prix > ceiling) return best;
    return !best || cell.prix > best.prix ? cell : best;
  }, null);
};

/**
 * Tarification d'une mesure HORS GRILLE (double validation côté configurateur).
 *
 * 1. Case de référence : sur l'axe qui tient dans la grille on garde l'arrondi
 *    supérieur habituel ; sur l'axe qui déborde on retient le tarif LE PLUS
 *    ÉLEVÉ de la ligne (ou de la colonne) — plusieurs feuilles ont une case
 *    extrême moins chère qu'une case plus petite.
 * 2. Hausse automatique : le dépassement est facturé au €/cm marginal de fin de
 *    grille (médiane des derniers pas positifs), largeur et hauteur séparément.
 *
 * Identique à `getPriceForMm` tant que la mesure tient dans la grille.
 */
export function getFallbackPriceForMm(sheetName, heightMm, widthMm) {
  const inGrid = resolveGridEntry(sheetName, heightMm, widthMm);
  ...
  const heightsAtLeast = data.heights.filter((height) => height >= heightCm);
  const clampedHeight = heightsAtLeast.length === 0;
  const rowHeight = clampedHeight ? Math.max(...data.heights) : Math.min(...heightsAtLeast);

  const row = data.prices.filter((entry) => entry.h === rowHeight);
  if (row.length === 0) return null;

  // ── Axe largeur ────────────────────────────────────────────────────────
  const widthsAtLeast = row.filter((entry) => entry.l >= widthCm);
  const clampedWidth = widthsAtLeast.length === 0;
  const widthCell = clampedWidth
    ? mostExpensiveCell(
        row,
        row.reduce((widest, entry) => (entry.l > widest.l ? entry : widest))
      )
    : widthsAtLeast.reduce((narrowest, entry) => (entry.l < narrowest.l ? entry : narrowest));
  if (!widthCell) return null;

  // ── Axe hauteur ────────────────────────────────────────────────────────
  const column = data.prices.filter((entry) => entry.l === widthCell.l);
  const reference = clampedHeight
    ? mostExpensiveCell(
        column,
        column.reduce((tallest, entry) => (entry.h > tallest.h ? entry : tallest))
      ) || widthCell
    : widthCell;

  const widthOverflowCm = Math.max(0, widthCm - reference.l);
  const heightOverflowCm = Math.max(0, heightCm - reference.h);

  const widthRatePerCm = marginalRatePerCm(
    data.prices
      .filter((entry) => entry.h === reference.h)
      .map((entry) => ({ cote: entry.l, prix: entry.prix }))
  );
  const heightRatePerCm = marginalRatePerCm(
    data.prices
      .filter((entry) => entry.l === reference.l)
      .map((entry) => ({ cote: entry.h, prix: entry.prix }))
  );

  return {
    price: roundCurrency(
      reference.prix + widthOverflowCm * widthRatePerCm + heightOverflowCm * heightRatePerCm
    ),
    gridPrice: reference.prix,
    billedHeight: reference.h,
    billedWidth: reference.l,
    clampedHeight,
    clampedWidth,
    widthOverflowCm,
    heightOverflowCm,
    widthRatePerCm: roundCurrency(widthRatePerCm),
    heightRatePerCm: roundCurrency(heightRatePerCm),
  };
}
...
export function calculateWasteManagementForItems(cartItems = []) {
  return cartItems.reduce(
    (accumulator, item) => {
      ...
      const categoryId = getProductFamilyCategory(item.productId);
...
/**
 * Tarif de pose SPÉCIFIQUE saisi sur une ligne (`posePriceOverride`) :
 * nombre positif arrondi au centime, ou null (= tarif catalogue). Toute valeur
 * vide ou invalide vaut null : jamais de pose négative ni de NaN.
 */
export const normalizePosePriceOverride = (value) => {
  if (value === null || value === undefined || value === '') return null;
  const amount = Number(value);
  return Number.isFinite(amount) && amount >= 0 ? roundCurrency(amount) : null;
};

/** Tarif de pose CATALOGUE d'un article (par unité, HT), hors tarif spécifique. */
export function getCataloguePosePrice(item) {
  if (!item) return 0;
  // Volet solaire : pose forfaitaire, quel que soit le tarif « volet ».
  if (item.productId === 'volet-solaire') return 100;
  ...
  return getPosePriceForType(productType);
}

/**
 * Tarif de pose EFFECTIF d'un article (par unité, HT) : le tarif spécifique de
 * la ligne s'il existe, sinon le tarif catalogue. Source unique pour le calcul
 * du devis, le récapitulatif, le PDF et l'export comptable.
 */
export function getItemPosePrice(item) {
  const override = normalizePosePriceOverride(item?.posePriceOverride);
  return override ?? getCataloguePosePrice(item);
}
...
export function calculateItemPrice(item) {
  ...
  // Composé ajouté hors grille (double validation) : les châssis sans tarif
  // fournisseur comptent pour 0 €, le prix de vente venant de la marge nette.
  const compositeBasePrice = item.outOfGridPricing
    ? compositePricing?.forcedTotalPrice
    : compositePricing?.totalPrice;
  let basePrice = compositeBasePrice ?? item.unitPrice ?? 0;
  let optionsPrice = 0;
  const pricing = getCataloguePricing();

  // Menuiserie cintrée (catégorie « Formes spéciales ») : en cintre intégré
  // (sous-traitance) le prix total saisi remplace TOUT le calcul automatique
  // (coefficient, coloration, vitrage, options). En imposte, la partie basse
  // se chiffre normalement et le prix du fixe cintré s'ajoute aux options.
  const cintrage = !item.isComposite ? normalizeCintrage(item) : null;
  const isCintrageManualPrice = cintrage?.type === 'integre';

  if (isCintrageManualPrice) {
    ({ basePrice, optionsPrice } = applyCintrageToPricing({ cintrage, basePrice, optionsPrice }));
  }

  if (!item.isComposite && !isCintrageManualPrice) {
    ...
    const hasGlazingContext =
      item.glazingOption && item.sheetName && item.widthMm && item.heightMm;
    const normalizedItemSheetName = hasGlazingContext
      ? normalizeSheetName(item.sheetName)
      : null;
    const frameSystem = hasGlazingContext
      ? getFrameSystemForProduct(normalizedItemSheetName)
      : null;
    const glassAreas =
    ...
    const fillingPricing =
      hasGlazingContext && glassAreas
        ? calculateGlazingAndPanelExtras({
            ...
            hasSousBassement: Boolean(item.hasSousBassement),
            sousBassementHeightMm: Number(item.sousBassementHeight || 0),
            hasTraverse: Boolean(item.hasTraverse),
            colorOptionId: item.colorOption?.id || 'blanc',
            isAlu:
              item.material === 'alu' ||
              /-alu$/i.test(item.productId || '') ||
              /\bALU\b/i.test(item.sheetName || ''),
          })
        : {
  ...
  if (
    ...
    // Services catalogue (métrage, forfait déplacement) et pièces détachées :
    // prix HT fixe (déjà achat × coefficient pour les pièces, cf.
    // lib/spare-parts.js), sans coefficient supplémentaire, remise, commission
    // ni pose. PU arrondi au centime PUIS multiplié par la quantité :
    // « PU affiché × Qté = Total ».
  ...
  const posePrice = item.includePose ? getItemPosePrice(item) : 0;
  ...
  // Prix « avant remise » (barré) cohérent avec la commission : celui qui,
  // remisé du pourcentage de la ligne, redonne EXACTEMENT le net commission
  // comprise. La commission est donc intégrée au barré majorée de 1/(1 - r),
  // et non ajoutée telle quelle : sinon la remise affichée (calculée sur le
  // prix hors commission) ne représentait plus le pourcentage annoncé
  // (ex. constaté : 546,79 / 2 816,52 = 19,4 % pour « -20 % »). Le net perçu
  // ne change pas : la commission reste intégralement encaissée.
  const discountRate = item.remise > 0 && item.remise < 100 ? item.remise / 100 : 0;
  let unitBeforeDiscount = roundCurrency(totalPriceBeforeDiscount);
  if (commissionUnit > 0) {
    unitBeforeDiscount =
      discountRate > 0
        ? roundCurrency(unitAfterCommission / (1 - discountRate))
        : unitAfterCommission;
  }

  return {
    unitPriceBase: roundCurrency(basePrice),
    unitPriceWithOptions: unitBeforeDiscount,
...
// Un appelant peut encore passer un nombre nu (ancien contrat, avant le choix
// %/€) : interprété comme un pourcentage, comme c'était le seul cas possible.
function normalizeCommissionInput(commission) {
  if (commission && typeof commission === 'object') {
    const mode = commission.mode === 'amount' ? 'amount' : 'percent';
    return {
      mode,
      percent: Math.max(0, Math.min(100, Number(commission.percent) || 0)),
      amount: Math.max(0, Number(commission.amount) || 0),
    };
  }
  return {
    mode: 'percent',
    percent: Math.max(0, Math.min(100, Number(commission) || 0)),
    amount: 0,
  };
}
 ...
 *
 * Deux modes : `percent` (montant = base × pourcentage) ou `amount` (montant
 * fixe saisi en €, indépendant de la base). Dans les deux cas, le montant de
 * commission obtenu est ensuite réparti AU PRORATA du prix HT après remise de
 * chaque ligne menuiserie éligible (pose et services exclus) — même
 * répartition, seule la façon d'obtenir le montant total change — puis
 * converti en majoration par unité stockée dans `commissionUnitHT`. Le champ
 * est éphémère (jamais persisté) : recalculé partout depuis les réglages du
 * devis.
 *
 * @param {Array} cartItems
 * @param {number|{mode: 'percent'|'amount', percent?: number, amount?: number}} commission
 * @returns {Array} nouvelle liste d'articles (avec `commissionUnitHT` posé/annulé)
 */
export function applyCommissionToCartItems(cartItems, commission) {
  const items = Array.isArray(cartItems) ? cartItems : [];
  const { mode, percent, amount } = normalizeCommissionInput(commission);

  const stripCommission = (item) =>
    item && item.commissionUnitHT ? { ...item, commissionUnitHT: 0 } : item;

  const hasCommission = mode === 'amount' ? amount > 0 : percent > 0;
  if (!hasCommission) return items.map(stripCommission);
  ...
  // utile seulement en mode pourcentage, mais toujours calculée : elle sert
  // aussi à repérer les lignes éligibles ensuite.
  let baseTotal = 0;
  ...
  const commissionAmount =
    mode === 'amount' ? roundCurrency(amount) : roundCurrency((baseTotal * percent) / 100);
  ...
  const isEligible = (item, calc) =>
    (Number(calc.totalLine) || 0) > 0 &&
    !NON_MENUISERIE.has(item?.productId) &&
    !SPARE_PART_PRODUCT_IDS.has(item?.productId);
