...
CATEGORIES.forEach((category) => {
  category.products.forEach((product) => {
    if (product.material !== 'alu' || !product.aluOf) return;
    const aluSheet = CANONICAL_SHEET_BY_PRODUCT_ID[product.id] || product.sheet;
    const sourceSheet = CANONICAL_SHEET_BY_PRODUCT_ID[product.aluOf];
    const source = sourceSheet ? pricingData[sourceSheet] : null;
    if (!source || PRICING[aluSheet]) return;
    PRICING[aluSheet] = {
...
export function getPoseLabel(item) {
  ...
  const base = item?.productLabel || item?.sheetName || '';
...
export function calculateItemPrice(item) {
  ...
  if (item.isComposite && item.voletMonobloc) {
    const dimensions = getCompositeDimensions(item.composition, item.modules);
    const voletPriceData = getPriceForMm(
      'Volet Filaire',
  ...
  const productType = item.isComposite ? 'composite' : getProductType(item.sheetName);
